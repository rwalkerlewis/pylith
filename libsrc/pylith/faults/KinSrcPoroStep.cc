...
void pylith::faults::KinSrcPoroStep::thicknessFn(const PylithInt dim,
...
void pylith::faults::KinSrcPoroStep::porosityFn(const PylithInt dim,
                                                const PylithInt numS,
                                                const PylithInt numA,
                                                const PylithInt sOff[],
                                                const PylithInt sOff_x[],
...
void pylith::faults::KinSrcPoroStep::beta_pFn(const PylithInt dim,
                                              const PylithInt numS,
                                              const PylithInt numA,
                                              const PylithInt sOff[],
                                              const PylithInt sOff_x[],
...
void pylith::faults::KinSrcPoroStep::beta_sigmaFn(const PylithInt dim,
                                                  const PylithInt numS,
                                                  const PylithInt numA,
                                                  const PylithInt sOff[],
                                                  const PylithInt sOff_x[],
...
void pylith::faults::KinSrcPoroStep::fault_permeabilityFn(const PylithInt dim,
                                                          const PylithInt numS,
                                                          const PylithInt numA,
                                                          const PylithInt sOff[],
                                                          const PylithInt sOff_x[],
...
void pylith::faults::KinSrcPoroStep::fluid_viscosityFn(const PylithInt dim,
                                                       const PylithInt numS,
                                                       const PylithInt numA,
                                                       const PylithInt sOff[],
                                                       const PylithInt sOff_x[],
...
void pylith::faults::KinSrcPoroStep::slipFn(const PylithInt dim,
                                            const PylithInt numS,
                                            const PylithInt numA,
                                            const PylithInt sOff[],
                                            const PylithInt sOff_x[],
...
void pylith::faults::KinSrcPoroStep::_auxiliaryFieldSetup(const spatialdata::units::Nondimensional &normalizer,
