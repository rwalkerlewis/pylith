...
// ----------------------------------------------------------------------
// Default constructor.
pylith::faults::KinSrc::KinSrc(void) : _auxiliaryFactory(new pylith::faults::KinSrcAuxiliaryFactory),
                                       _slipFnKernel(NULL),
...
void pylith::faults::KinSrc::deallocate(void)
{
...
void pylith::faults::KinSrc::originTime(const PylithReal value)
{
    _originTime = value;
} // originTime
...
void pylith::faults::KinSrc::auxFieldDB(spatialdata::spatialdb::SpatialDB *value)
{
    PYLITH_METHOD_BEGIN;
...
void pylith::faults::KinSrc::initialize(const pylith::topology::Field &faultAuxField,
                                        ...
                                        const spatialdata::geocoords::CoordSys *cs)
{
    ...
    delete _auxiliaryField;
    _auxiliaryField = new pylith::topology::Field(faultAuxField.getMesh());
    assert(_auxiliaryField);
...
void pylith::faults::KinSrc::updateSlip(PetscVec slipLocalVec,
...
void pylith::faults::KinSrc::updateSlipRate(PetscVec slipRateLocalVec,
                                            pylith::topology::Field *faultAuxiliaryField,
                                            const PylithScalar t,
                                            const PylithScalar timeScale)
{
...
void pylith::faults::KinSrc::updateSlipAcc(PetscVec slipAccLocalVec,
                                           pylith::topology::Field *faultAuxiliaryField,
                                           const PylithScalar t,
                                           const PylithScalar timeScale)
{
    ...
    err = PetscObjectCompose((PetscObject)faultAuxiliaryDM, "dmAux",
                             (PetscObject)_auxiliaryField->getDM());
    PYLITH_CHECK_ERROR(err);
    err = PetscObjectCompose((PetscObject)faultAuxiliaryDM, "A",
                             (PetscObject)_auxiliaryField->getLocalVector());
    PYLITH_CHECK_ERROR(err);
...
void pylith::faults::KinSrc::_setFEConstants(const pylith::topology::Field &faultAuxField) const
{
    PYLITH_METHOD_BEGIN;
    ...
    // the correct one.
    PetscDS prob = NULL;
    PetscDM dmAux = faultAuxField.getDM();
    assert(dmAux);
    PetscErrorCode err = DMGetDS(dmAux, &prob);
    PYLITH_CHECK_ERROR(err);
    assert(prob);
    ...
    err = PetscDSSetConstants(prob, numConstants, constants);
    PYLITH_CHECK_ERROR(err);
