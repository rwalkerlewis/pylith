...
// ----------------------------------------------------------------------
// Setup subfield query in auxiliary factory for bulk modulus from density, Vs, and Vp.
void pylith::materials::Query::biotModulusFromInput(const char *subfieldName,
                                                    pylith::feassemble::AuxiliaryFactory *factory)
{
                                            ...
                                            const pylith::int_array dbIndices)
{
    ...
    assert(dbIndices[i_density] < dbValues.size());
    const size_t i_vs = 1;
    assert(dbIndices[i_vs] < dbValues.size());
                                           ...
                                           const pylith::int_array dbIndices)
{
    ...
    assert(dbIndices[i_vs] < dbValues.size());
    const size_t i_vp = 2;
    assert(dbIndices[i_vp] < dbValues.size());
    const PylithScalar density = dbValues[dbIndices[i_density]];
    const PylithScalar vs = dbValues[dbIndices[i_vs]];
                                           ...
                                           const pylith::int_array dbIndices)
{
    ...
    assert(dbIndices[i_viscosity] < dbValues.size());
    const PylithScalar density = dbValues[dbIndices[i_density]];
    const PylithScalar vs = dbValues[dbIndices[i_vs]];
                                                       ...
                                                       const pylith::int_array dbIndices)
{
    ...
    assert(dbIndices[i_vs] < dbValues.size());
    const size_t i_viscosity = 2;
    assert(dbIndices[i_viscosity] + 2 < dbValues.size());
    const size_t i_shearModulusRatio = 5;
    assert(dbIndices[i_shearModulusRatio] + 2 < dbValues.size());
                                              ...
                                              const pylith::int_array dbIndices)
{
    ...
    assert(dbIndices[i_fluid_bulk_modulus] < dbValues.size());
    const size_t i_solid_bulk_modulus = 1;
    assert(dbIndices[i_solid_bulk_modulus] < dbValues.size());
    const size_t i_biot_coefficient = 2;
    assert(dbIndices[i_biot_coefficient] < dbValues.size());
    const size_t i_porosity = 3;
    assert(dbIndices[i_porosity] < dbValues.size());
