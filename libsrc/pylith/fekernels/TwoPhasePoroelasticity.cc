...
#include "pylith/fekernels/TwoPhasePoroelasticity.hh"
...
void
pylith::fekernels::TwoPhasePoroelasticity::g0v_grav(const PylithInt dim,
    ...
    const PylithInt i_saturations       = 5;


    const PylithScalar solid_density    = a[aOff[i_solid_density]];
    const PylithScalar* fluid_densities = &a[aOff[i_fluid_density]];
    const PylithScalar porosity         = a[aOff[i_porosity]];
    const PylithScalar* gravityField    = &a[aOff[i_gravityField]];
    const PylithScalar* saturations     = &a[aOff[i_saturations]];

    const PylithScalar fluid_density = saturations[0]*fluid_densities[0] + saturations[1]*fluid_densities[1];

    const PylithScalar bulkDensity = (1 - porosity) * a[aOff[i_solid_density]] + porosity * fluid_density;
...
void
pylith::fekernels::TwoPhasePoroelasticity::g0v_bodyforce(const PylithInt dim,
                                   const PylithInt numS,
                                   const PylithInt numA,
                                   const PylithInt sOff[],
                                   const PylithInt sOff_x[],
...
void
pylith::fekernels::TwoPhasePoroelasticity::g0v_gravbodyforce(const PylithInt dim,
                                                     const PylithInt numS,
                                                     const PylithInt numA,
                                                     const PylithInt sOff[],
                                                     const PylithInt sOff_x[],
    ...
    const PylithInt i_saturations = 6;
...
void
pylith::fekernels::TwoPhasePoroelasticity::g0p_source(const PylithInt dim,
                                             const PylithInt numS,
                                             const PylithInt numA,
                                             const PylithInt sOff[],
                                             const PylithInt sOff_x[],
...
void
pylith::fekernels::TwoPhasePoroelasticity::g0p_sourceDensity(const PylithInt dim,
                                                                       const PylithInt numS,
                                                                       const PylithInt numA,
                                                                       const PylithInt sOff[],
                                                                       const PylithInt sOff_x[],
    ...
    pylith::fekernels::TwoPhasePoroelasticity::g0p_source(dim, _numS, numASource,
...
// g0p function for isotropic linear Poroelasticity plane strain with source density, gravity, and body force.
void
pylith::fekernels::TwoPhasePoroelasticity::g0p_sourceDensity_grav(const PylithInt dim,
                                                                       const PylithInt numS,
                                                                       const PylithInt numA,
                                                                       const PylithInt sOff[],
                                                                       const PylithInt sOff_x[],
...
// g0p function for isotropic linear Poroelasticity plane strain with source density, gravity, and body force.
void
pylith::fekernels::TwoPhasePoroelasticity::g0p_sourceDensity_body(const PylithInt dim,
                                                                       const PylithInt numS,
                                                                       const PylithInt numA,
                                                                       const PylithInt sOff[],
                                                                       const PylithInt sOff_x[],
...
// g0p function for isotropic linear Poroelasticity plane strain with source density, gravity, and body force.
void
pylith::fekernels::TwoPhasePoroelasticity::g0p_sourceDensity_grav_body(const PylithInt dim,
                                                                       const PylithInt numS,
                                                                       const PylithInt numA,
                                                                       const PylithInt sOff[],
                                                                       const PylithInt sOff_x[],
...
void
pylith::fekernels::TwoPhasePoroelasticity::g0e(const PylithInt dim,
                                       const PylithInt numS,
                                       const PylithInt numA,
                                       const PylithInt sOff[],
                                       const PylithInt sOff_x[],
...
void
pylith::fekernels::TwoPhasePoroelasticity::Jg0ee(const PylithInt dim,
                                                const PylithInt numS,
                                                const PylithInt numA,
                                                const PylithInt sOff[],
                                                const PylithInt sOff_x[],
...
void
pylith::fekernels::TwoPhasePoroelasticity::Jg1eu(const PylithInt dim,
                                                const PylithInt numS,
                                                const PylithInt numA,
                                                const PylithInt sOff[],
                                                const PylithInt sOff_x[],
...
void
pylith::fekernels::TwoPhasePoroelasticityPlaneStrain::cauchyStrain(const PylithInt dim,
                                                       const PylithInt numS,
                                                       const PylithInt numA,
                                                       const PylithInt sOff[],
                                                       const PylithInt sOff_x[],
...
void
pylith::fekernels::TwoPhasePoroelasticity3D::cauchyStrain(const PylithInt dim,
                                                       const PylithInt numS,
                                                       const PylithInt numA,
                                                       const PylithInt sOff[],
                                                       const PylithInt sOff_x[],
