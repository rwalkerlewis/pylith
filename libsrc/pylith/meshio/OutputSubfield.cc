...
// ------------------------------------------------------------------------------------------------
// Constructor
pylith::meshio::OutputSubfield::OutputSubfield(void) : _dm(NULL),
                                                       _vector(NULL),
...
void pylith::meshio::OutputSubfield::deallocate(void)
{
    PetscErrorCode err;
    err = DMDestroy(&_dm);
    PYLITH_CHECK_ERROR(err);
    err = VecDestroy(&_vector);
    PYLITH_CHECK_ERROR(err);
} // deallocate
                                       ...
                                       const int basisOrder)
{
    ...
    OutputSubfield *subfield = new OutputSubfield();
    assert(subfield);
    ...
    PetscErrorCode err;
    err = DMClone(mesh.getDM(), &subfield->_dm);
    PYLITH_CHECK_ERROR(err);
    err = PetscObjectSetName((PetscObject)subfield->_dm, name);
    PYLITH_CHECK_ERROR(err);

    PetscFE fe = pylith::topology::FieldOps::createFE(subfield->_discretization, subfield->_dm,
                                                      info.description.numComponents);
    assert(fe);
    err = PetscFESetName(fe, info.description.label.c_str());
    PYLITH_CHECK_ERROR(err);
    err = DMSetField(subfield->_dm, 0, NULL, (PetscObject)fe);
    PYLITH_CHECK_ERROR(err);
    err = DMSetFieldAvoidTensor(subfield->_dm, 0, PETSC_TRUE);
    PYLITH_CHECK_ERROR(err);
    err = PetscFEDestroy(&fe);
    PYLITH_CHECK_ERROR(err);
    err = DMCreateDS(subfield->_dm);
    PYLITH_CHECK_ERROR(err);

    err = DMCreateGlobalVector(subfield->_dm, &subfield->_vector);
    PYLITH_CHECK_ERROR(err);
    err = PetscObjectSetName((PetscObject)subfield->_vector, name);
    PYLITH_CHECK_ERROR(err);
                                       ...
                                       const char *name)
{
    ...
    err = PetscSectionClone(fieldVisitor.localSection(), &subfieldSection);
    PYLITH_CHECK_ERROR(err);
    err = PetscSectionGetChart(fieldVisitor.localSection(), &pStart, &pEnd);
    PYLITH_CHECK_ERROR(err);
    for (PetscInt point = pStart, offset = 0; point < pEnd; ++point)
    {
        const PetscInt numDof = fieldVisitor.sectionDof(point);
        err = PetscSectionSetOffset(subfieldSection, point, offset);
        PYLITH_CHECK_ERROR(err);
        err = PetscSectionSetDof(subfieldSection, point, numDof);
        PYLITH_CHECK_ERROR(err);
        offset += numDof;
    } // for
    err = DMSetLocalSection(subfield->_dm, subfieldSection);
    PYLITH_CHECK_ERROR(err);
    err = PetscSectionDestroy(&subfieldSection);
    PYLITH_CHECK_ERROR(err);
    err = DMCreateGlobalVector(subfield->_dm, &subfield->_vector);
    PYLITH_CHECK_ERROR(err);
    err = PetscObjectSetName((PetscObject)subfield->_vector, name);
...
int pylith::meshio::OutputSubfield::getBasisOrder(void) const
{
...
void pylith::meshio::OutputSubfield::project(const PetscVec &fieldVector)
{
    PYLITH_METHOD_BEGIN;
    assert(fieldVector);
    assert(_vector);
...
void pylith::meshio::OutputSubfield::extractSubfield(const pylith::topology::Field &field,
                                                     const PetscInt subfieldIndex)
{
    ...
    err = PetscSectionGetField(field.getLocalSection(), subfieldIndex, &subfieldSection);
    PYLITH_CHECK_ERROR(err);
    err = PetscSectionGetStorageSize(subfieldSection, &storageSize);
    PYLITH_CHECK_ERROR(err);

    PetscVec subfieldVector = this->getVector();
    PetscInt subfieldSize = 0;
    err = VecGetLocalSize(subfieldVector, &subfieldSize);
    PYLITH_CHECK_ERROR(err);
    ...
    err = VecGetArray(subfieldVector, &subfieldArray);
    PYLITH_CHECK_ERROR(err);
    ...
    err = VecRestoreArray(subfieldVector, &subfieldArray);
    PYLITH_CHECK_ERROR(err);
