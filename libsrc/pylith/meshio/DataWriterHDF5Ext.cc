...
// ----------------------------------------------------------------------
// Constructor
pylith::meshio::DataWriterHDF5Ext::DataWriterHDF5Ext(void) : _filename("output.h5"),
                                                             _h5(new HDF5),
                                                             _tstampIndex(0)
{ // constructor
} // constructor
...
void pylith::meshio::DataWriterHDF5Ext::deallocate(void)
{
    PYLITH_METHOD_BEGIN;
    ...
    for (dataset_type::iterator d_iter = _datasets.begin();
         ...
         ++d_iter)
    {
        err = PetscViewerDestroy(&d_iter->second.viewer);
...
// ----------------------------------------------------------------------
// Copy constructor.
pylith::meshio::DataWriterHDF5Ext::DataWriterHDF5Ext(const DataWriterHDF5Ext &w) : DataWriter(w),
...
void pylith::meshio::DataWriterHDF5Ext::open(const pylith::topology::Mesh &mesh,
                                             const bool isInfo)
{
    PYLITH_METHOD_BEGIN;
    ...
    try
    {
        ...
        PetscDM dmMesh = mesh.getDM();
        assert(dmMesh);
        MPI_Comm comm;
        PetscMPIInt commRank;
        PetscErrorCode err = PetscObjectGetComm((PetscObject)dmMesh, &comm);
        PYLITH_CHECK_ERROR(err);
        ...
        err = PetscViewerBinaryOpen(comm, filenameVertices.c_str(), FILE_MODE_WRITE, &binaryViewer);
        PYLITH_CHECK_ERROR(err);
        ...
        PetscBool isseq;
        err = PetscObjectTypeCompare((PetscObject)coordsGlobalVec, VECSEQ, &isseq);
        PYLITH_CHECK_ERROR(err);
        if (isseq)
        {
            err = VecView_Seq(coordsGlobalVec, binaryViewer);
            PYLITH_CHECK_ERROR(err);
        }
        else
        {
            err = VecView_MPI(coordsGlobalVec, binaryViewer);
            PYLITH_CHECK_ERROR(err);
        } // if/else
        err = VecDestroy(&coordsGlobalVec);
        PYLITH_CHECK_ERROR(err);
        err = PetscViewerDestroy(&binaryViewer);
        PYLITH_CHECK_ERROR(err);

        PetscInt vStart, vEnd;
        PetscInt n, numVerticesLocal = 0, numVertices;
        PetscIS globalVertexNumbers = NULL;
        err = DMPlexGetDepthStratum(dmMesh, 0, &vStart, &vEnd);
        PYLITH_CHECK_ERROR(err);
        err = DMPlexGetVertexNumbering(dmMesh, &globalVertexNumbers);
        PYLITH_CHECK_ERROR(err);
        err = ISGetLocalSize(globalVertexNumbers, &n);
        PYLITH_CHECK_ERROR(err);
        if (n > 0)
        {
            const PetscInt *indices = NULL;
            err = ISGetIndices(globalVertexNumbers, &indices);
            PYLITH_CHECK_ERROR(err);
            ...
            err = ISRestoreIndices(globalVertexNumbers, &indices);
            PYLITH_CHECK_ERROR(err);
        } // if
        err = MPI_Allreduce(&numVerticesLocal, &numVertices, 1, MPI_INT, MPI_SUM, comm);
        PYLITH_CHECK_ERROR(err);
        ...
        if (!commRank)
        {
            ...
            hsize_t dims[ndims];
            dims[0] = numVertices;
            const spatialdata::geocoords::CoordSys *cs = mesh.getCoordSys();
            assert(cs);
        ...
        err = DMPlexGetVTKCellHeight(dmMesh, &cellHeight);
        PYLITH_CHECK_ERROR(err);
        err = DMPlexGetHeightStratum(dmMesh, cellHeight, &cStart, &cEnd);
        PYLITH_CHECK_ERROR(err);
        for (PetscInt cell = cStart; cell < cEnd; ++cell)
        {
            ...
            if (pylith::topology::MeshOps::isCohesiveCell(dmMesh, cell))
            {
                continue;
            }
            err = DMPlexGetTransitiveClosure(dmMesh, cell, PETSC_TRUE, &closureSize, &closure);
            PYLITH_CHECK_ERROR(err);
            ...
            err = DMPlexRestoreTransitiveClosure(dmMesh, cell, PETSC_TRUE, &closureSize, &closure);
            PYLITH_CHECK_ERROR(err);
        } // for
        err = MPI_Allreduce(&numCornersLocal, &numCorners, 1, MPIU_INT, MPI_MAX, comm);
        PYLITH_CHECK_ERROR(err);
        ...
        err = DMPlexGetVertexNumbering(dmMesh, &globalVertexNumbers);
        PYLITH_CHECK_ERROR(err);
        err = ISGetIndices(globalVertexNumbers, &gvertex);
        PYLITH_CHECK_ERROR(err);
        err = VecCreate(comm, &cellVec);
        PYLITH_CHECK_ERROR(err);
        err = VecSetSizes(cellVec, conesSize, PETSC_DETERMINE);
        PYLITH_CHECK_ERROR(err);
        err = VecSetBlockSize(cellVec, numCorners);
        PYLITH_CHECK_ERROR(err);
        err = VecSetFromOptions(cellVec);
        PYLITH_CHECK_ERROR(err);
        err = PetscObjectSetName((PetscObject)cellVec, "cells");
        PYLITH_CHECK_ERROR(err);
        err = VecGetArray(cellVec, &vertices);
        PYLITH_CHECK_ERROR(err);
        for (PetscInt cell = cStart, v = 0; cell < cEnd; ++cell)
        {
            ...
            DMPolytopeType ct;
            err = DMPlexGetCellType(dmMesh, cell, &ct);
            PYLITH_CHECK_ERROR(err);
            err = DMPlexInvertCell(ct, closure);
            PYLITH_CHECK_ERROR(err);
            for (p = 0; p < nC; ++p)
            {
        ...
        err = ISRestoreIndices(globalVertexNumbers, &gvertex);
        PYLITH_CHECK_ERROR(err);
        err = VecRestoreArray(cellVec, &vertices);
        PYLITH_CHECK_ERROR(err);
        err = VecGetSize(cellVec, &numCells);
        PYLITH_CHECK_ERROR(err);
        numCells /= numCorners;

        const std::string &filenameCells = _datasetFilename("cells");
        err = PetscViewerBinaryOpen(comm, filenameCells.c_str(), FILE_MODE_WRITE, &binaryViewer);
        PYLITH_CHECK_ERROR(err);
        err = PetscViewerBinarySetSkipHeader(binaryViewer, PETSC_TRUE);
        PYLITH_CHECK_ERROR(err);
#if 0
...
#else
        err = PetscObjectTypeCompare((PetscObject)cellVec, VECSEQ, &isseq);
        PYLITH_CHECK_ERROR(err);
        if (isseq)
        {
            err = VecView_Seq(cellVec, binaryViewer);
            PYLITH_CHECK_ERROR(err);
        }
        else
        {
            err = VecView_MPI(cellVec, binaryViewer);
            PYLITH_CHECK_ERROR(err);
        }
#endif
        err = VecDestroy(&cellVec);
        PYLITH_CHECK_ERROR(err);
        err = PetscViewerDestroy(&binaryViewer);
        PYLITH_CHECK_ERROR(err);
...
// ----------------------------------------------------------------------
// Close output files.
void pylith::meshio::DataWriterHDF5Ext::close(void)
{
    PYLITH_METHOD_BEGIN;
...
void pylith::meshio::DataWriterHDF5Ext::writeVertexField(const PylithScalar t,
                                                         const pylith::meshio::OutputSubfield &subfield)
{
    ...
    try
    {
        PetscDM dmMesh = subfield.getDM();
        assert(dmMesh);
        PetscErrorCode err;

        MPI_Comm comm;
        PetscMPIInt commRank;
        err = PetscObjectGetComm((PetscObject)dmMesh, &comm);
        PYLITH_CHECK_ERROR(err);
        err = MPI_Comm_rank(comm, &commRank);
        PYLITH_CHECK_ERROR(err);
        ...
        {
            err = PetscViewerBinaryOpen(comm, _datasetFilename(name).c_str(), FILE_MODE_WRITE, &binaryViewer);
            PYLITH_CHECK_ERROR(err);
            err = PetscViewerBinarySetSkipHeader(binaryViewer, PETSC_TRUE);
            PYLITH_CHECK_ERROR(err);
        ...
        PetscVec vector = subfield.getVector();
        assert(vector);
        PetscBool isseq;
        err = PetscObjectTypeCompare((PetscObject)vector, VECSEQ, &isseq);
        PYLITH_CHECK_ERROR(err);
        if (isseq)
        {
            err = VecView_Seq(vector, binaryViewer);
            PYLITH_CHECK_ERROR(err);
        }
        else
        {
            err = VecView_MPI(vector, binaryViewer);
            PYLITH_CHECK_ERROR(err);
        } // if/else
        ...
        if (createdExternalDataset)
        {
            ...
            err = VecGetDM(vector, &dm);
            PYLITH_CHECK_ERROR(err);
            assert(dm);
            err = DMGetSection(dm, &section);
            PYLITH_CHECK_ERROR(err);
            assert(section);

            err = DMPlexGetDepthStratum(dmMesh, 0, &vStart, NULL);
            PYLITH_CHECK_ERROR(err);
            err = DMPlexGetVertexNumbering(dmMesh, &globalVertexNumbers);
            PYLITH_CHECK_ERROR(err);
            err = ISGetLocalSize(globalVertexNumbers, &n);
            ...
            if (n > 0)
            {
                ...
                err = ISGetIndices(globalVertexNumbers, &indices);
                PYLITH_CHECK_ERROR(err);
                err = PetscSectionGetDof(section, vStart, &dof);
                PYLITH_CHECK_ERROR(err);
                for (PetscInt v = 0; v < n; ++v)
                {
                    if (indices[v] >= 0)
            ...
            int fiberDim = 0;
            err = MPI_Allreduce(&fiberDimLocal, &fiberDim, 1, MPI_INT, MPI_MAX, comm);
            PYLITH_CHECK_ERROR(err);
            err = MPI_Allreduce(&numVerticesLocal, &numVertices, 1, MPI_INT, MPI_SUM, comm);
            PYLITH_CHECK_ERROR(err);
            assert(fiberDim > 0);
...
void pylith::meshio::DataWriterHDF5Ext::writeCellField(const PylithScalar t,
                                                       const pylith::meshio::OutputSubfield &subfield)
{
    PYLITH_METHOD_BEGIN;
    assert(_h5);
    ...
    try
    {
        ...
        if (createdExternalDataset)
        {
            ...
            PetscSection section = NULL;
            err = DMGetLocalSection(subfield.getDM(), &section);
            assert(section);
            ...
            err = DMPlexGetHeightStratum(dmMesh, cellHeight, &cStart, &cEnd);
            PYLITH_CHECK_ERROR(err);
            err = DMPlexGetCellNumbering(dmMesh, &globalCellNumbers);
            PYLITH_CHECK_ERROR(err);
            err = ISGetLocalSize(globalCellNumbers, &numCells);
            PYLITH_CHECK_ERROR(err);
            if (numCells > 0)
            {
                const PetscInt *indices = NULL;
                err = ISGetIndices(globalCellNumbers, &indices);
                PYLITH_CHECK_ERROR(err);
                err = PetscSectionGetDof(section, cStart, &dof);
                PYLITH_CHECK_ERROR(err);
                ...
                err = ISRestoreIndices(globalCellNumbers, &indices);
                PYLITH_CHECK_ERROR(err);
            } // if
            int fiberDimLocal = dof;
            int fiberDim = 0;
            MPI_Allreduce(&fiberDimLocal, &fiberDim, 1, MPI_INT, MPI_MAX, comm);
            err = MPI_Allreduce(&numLocalCells, &numCells, 1, MPI_INT, MPI_SUM, comm);
            PYLITH_CHECK_ERROR(err);
            assert(fiberDim > 0);
            assert(numCells > 0);
...
void pylith::meshio::DataWriterHDF5Ext::writePointNames(const pylith::string_vector &names,
                                                        const pylith::topology::Mesh &mesh)
{
    ...
    try
    {
        ...
        int nprocs = 0;
        mpierr = MPI_Comm_size(comm, &nprocs);
        assert(MPI_SUCCESS == mpierr);

        // Number of names on each process.
        const int numNamesLocal = names.size();
        int_array numNamesArray(nprocs);
        mpierr = MPI_Allgather((void *)&numNamesLocal, 1, MPI_INT, &numNamesArray[0], 1, MPI_INT, comm);
        assert(MPI_SUCCESS == mpierr);
        ...
        mpierr = MPI_Allreduce(&maxStringLengthLocal, &maxStringLength, 1, MPI_INT, MPI_MAX, comm);
        assert(MPI_SUCCESS == mpierr);
        ...
        if (!commRank)
        {
            namesFixedLength.resize(numNames * maxStringLength);
        }
...
void pylith::meshio::DataWriterHDF5Ext::_writeTimeStamp(const PylithScalar t)
{
    PYLITH_METHOD_BEGIN;
