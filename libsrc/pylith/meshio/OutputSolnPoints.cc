...
// ------------------------------------------------------------------------------------------------
// Constructor
pylith::meshio::OutputSolnPoints::OutputSolnPoints(void) : _pointMesh(NULL),
                                                           _pointSoln(NULL),
                                                           _interpolator(NULL)
{
...
void pylith::meshio::OutputSolnPoints::deallocate(void)
{
    PYLITH_METHOD_BEGIN;
    ...
    if (_interpolator)
    {
        PetscErrorCode err = DMInterpolationDestroy(&_interpolator);
...
void pylith::meshio::OutputSolnPoints::setPoints(const PylithReal *pointCoords,
                                                 const int numPoints,
                                                 const int spaceDim,
...
void pylith::meshio::OutputSolnPoints::_writeSolnStep(const PylithReal t,
                                                      ...
                                                      const pylith::topology::Field &solution)
{
    ...
    for (size_t iField = 0; iField < numSubfieldNames; iField++)
    {
        ...
        subfield = this->_getSubfield(*_pointSoln, *_pointMesh, subfieldNames[iField].c_str());
        assert(subfield);
...
// ------------------------------------------------------------------------------------------------
// Setup interpolator.
void pylith::meshio::OutputSolnPoints::_setupInterpolator(const pylith::topology::Field &solution)
{
    PYLITH_METHOD_BEGIN;
    ...
    if (_interpolator)
    {
        err = DMInterpolationDestroy(&_interpolator);
        PYLITH_CHECK_ERROR(err);
    } // if
    assert(!_interpolator);

    const spatialdata::geocoords::CoordSys *csMesh = solution.getMesh().getCoordSys();
    assert(csMesh);
    ...
    err = DMInterpolationCreate(comm, &_interpolator);
    PYLITH_CHECK_ERROR(err);
    err = DMInterpolationSetDim(_interpolator, spaceDim);
    PYLITH_CHECK_ERROR(err);
    err = DMInterpolationAddPoints(_interpolator, _pointNames.size(), (PetscReal *)&_pointCoords[0]);
    PYLITH_CHECK_ERROR(err);
    const PetscBool pointsAllProcs = PETSC_TRUE;
    const PetscBool ignoreOutsideDomain = PETSC_FALSE;
    err = DMInterpolationSetUp(_interpolator, dmSoln, pointsAllProcs, ignoreOutsideDomain);
    PYLITH_CHECK_ERROR(err);

    // Create mesh corresponding to local points.
    const size_t numPointsLocal = _interpolator->n;
    PylithScalar *pointsLocal = NULL;
    err = VecGetArray(_interpolator->coords, &pointsLocal);
    PYLITH_CHECK_ERROR(err);

    PylithReal lengthScale = 1.0;
    err = DMPlexGetScale(dmSoln, PETSC_UNIT_LENGTH, &lengthScale);
    PYLITH_CHECK_ERROR(err);
    ...
    err = DMInterpolationSetDof(_interpolator, numDof);
    PYLITH_CHECK_ERROR(err);

    delete _pointSoln;
    _pointSoln = new pylith::topology::Field(*_pointMesh);
...
void pylith::meshio::OutputSolnPoints::_interpolateField(const pylith::topology::Field &solution)
{
    PYLITH_METHOD_BEGIN;
    assert(_pointSoln);
...
void pylith::meshio::OutputSolnPoints::_writePointNames(void)
{
    PYLITH_METHOD_BEGIN;
