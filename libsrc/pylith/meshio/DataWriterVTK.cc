...
extern PetscErrorCode DMPlexVTKWriteAll(PetscObject odm,
                                        PetscViewer viewer);

// ------------------------------------------------------------------------------------------------
// Constructor
pylith::meshio::DataWriterVTK::DataWriterVTK(void) : _timeConstant(1.0),
                                                     _precision(6),
...
void pylith::meshio::DataWriterVTK::deallocate(void)
{ // deallocate
    PYLITH_METHOD_BEGIN;
...
// ------------------------------------------------------------------------------------------------
// Copy constructor.
pylith::meshio::DataWriterVTK::DataWriterVTK(const DataWriterVTK &w) : DataWriter(w),
...
void pylith::meshio::DataWriterVTK::timeConstant(const PylithScalar value)
{
    PYLITH_METHOD_BEGIN;
...
void pylith::meshio::DataWriterVTK::precision(const int value)
{
    PYLITH_METHOD_BEGIN;
...
void pylith::meshio::DataWriterVTK::open(const pylith::topology::Mesh &mesh,
                                         const bool isInfo)
{
    PYLITH_METHOD_BEGIN;
    ...
    _dm = mesh.getDM();
    assert(_dm);
    err = PetscObjectReference((PetscObject)_dm);
    PYLITH_CHECK_ERROR(err);
...
// ------------------------------------------------------------------------------------------------
// Close output files.
void pylith::meshio::DataWriterVTK::close(void)
{
    PYLITH_METHOD_BEGIN;
    ...
    {
        assert(_dm);
        PetscErrorCode err = DMDestroy(&_dm);
        PYLITH_CHECK_ERROR(err);
    } // if
...
void pylith::meshio::DataWriterVTK::openTimeStep(const PylithScalar t,
...
void pylith::meshio::DataWriterVTK::closeTimeStep(void)
{
    PYLITH_METHOD_BEGIN;
    ...
    if (_isOpenTimeStep)
    {
        ...
        err = DMHasLabel(_dm, "vtk", &hasLabel);
        PYLITH_CHECK_ERROR(err);
        if (hasLabel)
        {
            err = DMClearLabelStratum(_dm, "vtk", 1);
            PYLITH_CHECK_ERROR(err);
            err = DMClearLabelStratum(_dm, "vtk", 2);
            PYLITH_CHECK_ERROR(err);
        } // if
    }     // if
...
void pylith::meshio::DataWriterVTK::writeVertexField(const PylithScalar t,
                                                     const pylith::meshio::OutputSubfield &subfield)
{
    ...
    PetscErrorCode err;
    PetscViewerVTKFieldType ft = subfield.getDescription().vectorFieldType == pylith::topology::FieldBase::VECTOR ? PETSC_VTK_POINT_VECTOR_FIELD : PETSC_VTK_POINT_FIELD;
    err = PetscViewerVTKAddField(_viewer, (PetscObject)_dm, DMPlexVTKWriteAll, PETSC_DEFAULT, ft,
                                 PETSC_TRUE, (PetscObject)subfield.getVector());
    PYLITH_CHECK_ERROR(err);
    err = PetscObjectReference((PetscObject)subfield.getVector());
    PYLITH_CHECK_ERROR(err); // Viewer destroys Vec
...
void pylith::meshio::DataWriterVTK::writeCellField(const PylithScalar t,
                                                   const pylith::meshio::OutputSubfield &subfield)
{
    PYLITH_METHOD_BEGIN;
    ...
    PetscErrorCode err;
    PetscViewerVTKFieldType ft = subfield.getDescription().vectorFieldType == pylith::topology::FieldBase::VECTOR ? PETSC_VTK_CELL_VECTOR_FIELD : PETSC_VTK_CELL_FIELD;
    err = PetscViewerVTKAddField(_viewer, (PetscObject)_dm, DMPlexVTKWriteAll, PETSC_DEFAULT, ft,
                                 PETSC_TRUE, (PetscObject)subfield.getVector());
    PYLITH_CHECK_ERROR(err);
    err = PetscObjectReference((PetscObject)subfield.getVector());
