...
pylith::meshio::GMVFileBinary::GMVFileBinary(const char *filename,
                                             const bool flipEndian) : GMVFile(filename),
...
void pylith::meshio::GMVFileBinary::read(scalar_array *coordinates,
                                         int_array *cells,
...
void pylith::meshio::GMVFileBinary::write(const scalar_array &coordinates,
...
// ----------------------------------------------------------------------
void pylith::meshio::GMVFileBinary::_readHeader(std::ifstream &fin)
{ // _readHeader
    PYLITH_METHOD_BEGIN;
...
// ----------------------------------------------------------------------
void pylith::meshio::GMVFileBinary::_readVertices(std::ifstream &fin,
...
// ----------------------------------------------------------------------
void pylith::meshio::GMVFileBinary::_readCells(std::ifstream &fin,
                                               int_array *cells,
                                               int *numCells,
                                               int *numCorners)
{ // _readCells
...
// ----------------------------------------------------------------------
void pylith::meshio::GMVFileBinary::_readVariables(std::ifstream &fin,
                                                   const int numVertices,
                                                   const int numCells)
{ // _readVariables
    PYLITH_METHOD_BEGIN;
    ...
    while ("endvars" != varName && !fin.eof() && fin.good())
    {
        ...
        if (1 == varType)
        { // variable/attribute associated with vertices
            float_array vals(numVertices);
            fin.read((char *)&vals[0], sizeof(float) * numVertices);
        }
        else
        { // variable/attribute associated with cells
...
// ----------------------------------------------------------------------
void pylith::meshio::GMVFileBinary::_readFlags(std::ifstream &fin,
                                               const int numVertices,
                                               const int numCells)
{ // _readFlags
    PYLITH_METHOD_BEGIN;
...
// ----------------------------------------------------------------------
void pylith::meshio::GMVFileBinary::_readMaterials(std::ifstream &fin,
                                                   int_array *materialIds,
                                                   const int numVertices,
                                                   const int numCells)
{ // _readMaterials
    ...
    if (0 == dataType)
    { // materials associated with cells
    ...
    { // materials associated with vertices
        int_array buffer(numVertices);
        fin.read((char *)&buffer[0], sizeof(int) * numVertices);
    } // else
