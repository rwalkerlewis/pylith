...
void pylith::meshio::MeshBuilder::buildMesh(topology::Mesh *mesh,
...
{ // buildMesh
    ...
    err = MPI_Bcast(&dim, 1, MPIU_INT, 0, comm);
    PYLITH_CHECK_ERROR(err);
    err = MPI_Bcast(&spaceDim, 1, MPIU_INT, 0, comm);
    PYLITH_CHECK_ERROR(err);
    ...
    for (PetscInt coff = 0; coff < bound; coff += numCorners)
    {
        ...
        err = DMPlexInvertCell(ct, (int *)&cells[coff]);
        PYLITH_CHECK_ERROR(err);
    }
    err = DMPlexCreateFromCellListPetsc(comm, dim, numCells, numVertices, numCorners, interpolate, &cells[0], spaceDim, &(*coordinates)[0], &dmMesh);
    PYLITH_CHECK_ERROR(err);
    mesh->setDM(dmMesh);
