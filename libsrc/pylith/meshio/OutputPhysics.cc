...
void pylith::meshio::OutputPhysics::deallocate(void)
{
...
void pylith::meshio::OutputPhysics::setInfoFields(const char *names[],
                                                  const int numNames)
{
    PYLITH_METHOD_BEGIN;
...
void pylith::meshio::OutputPhysics::setDataFields(const char *names[],
                                                  const int numNames)
{
    PYLITH_METHOD_BEGIN;
...
// ------------------------------------------------------------------------------------------------
// Set time scale.
void pylith::meshio::OutputPhysics::setTimeScale(const PylithReal value)
{
...
void pylith::meshio::OutputPhysics::verifyConfiguration(const pylith::topology::Field &solution) const
{
    PYLITH_METHOD_BEGIN;
    ...
    if ((numDataFields > 0) && (std::string("all") != _dataFieldNames[0]))
    {
        ...
        {
            if (solution.hasSubfield(_dataFieldNames[i].c_str()))
            {
                continue;
            }
            if (auxiliaryField && auxiliaryField->hasSubfield(_dataFieldNames[i].c_str()))
            {
                continue;
            }
            if (derivedField && derivedField->hasSubfield(_dataFieldNames[i].c_str()))
            {
                continue;
            }
...
void pylith::meshio::OutputPhysics::update(const PylithReal t,
...
void pylith::meshio::OutputPhysics::_writeInfo(void)
{
    PYLITH_METHOD_BEGIN;
...
// ------------------------------------------------------------------------------------------------
// Prepare for output.
void pylith::meshio::OutputPhysics::_open(const pylith::topology::Mesh &mesh,
                                          const bool isInfo)
{
    PYLITH_METHOD_BEGIN;
...
// ------------------------------------------------------------------------------------------------
/// Close output files.
void pylith::meshio::OutputPhysics::_close(void)
{
    PYLITH_METHOD_BEGIN;
...
void pylith::meshio::OutputPhysics::_openDataStep(const PylithReal t,
...
void pylith::meshio::OutputPhysics::_closeDataStep(void)
{
    assert(_writer);
...
// ------------------------------------------------------------------------------------------------
// Write output for step in solution.
void pylith::meshio::OutputPhysics::_writeDataStep(const PylithReal t,
                                                   const PylithInt tindex,
                                                   const pylith::topology::Field &solution)
{
    PYLITH_METHOD_BEGIN;
    ...
    if (derivedField)
    {
        derivedField->scatterLocalToOutput();
    }
    PetscVec derivedVector = (derivedField) ? derivedField->getOutputVector() : NULL;

    PetscVec solutionVector = solution.getOutputVector();
    assert(solutionVector);
    ...
    for (size_t i = 0; i < numDataFields; i++)
    {
        ...
        {
            subfield = OutputObserver::_getSubfield(solution, domainMesh, dataNames[i].c_str());
            assert(subfield);
            subfield->project(solutionVector);
        }
        else if (auxiliaryField && auxiliaryField->hasSubfield(dataNames[i].c_str()))
        {
            subfield = OutputObserver::_getSubfield(*auxiliaryField, domainMesh, dataNames[i].c_str());
            assert(subfield);
            subfield->project(auxiliaryVector);
        }
        else if (derivedField && derivedField->hasSubfield(dataNames[i].c_str()))
        {
            subfield = OutputObserver::_getSubfield(*derivedField, domainMesh, dataNames[i].c_str());
            assert(subfield);
