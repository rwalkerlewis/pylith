...
// ---------------------------------------------------------------------------------------------------------------------
// Constructor
pylith::meshio::DataWriterHDF5::DataWriterHDF5(void) : _filename("output.h5"),
                                                       _viewer(0),
                                                       _tstamp(0),
                                                       _tstampIndex(0)
{
...
void pylith::meshio::DataWriterHDF5::deallocate(void)
{
    PYLITH_METHOD_BEGIN;
    ...
    err = PetscViewerDestroy(&_viewer);
    PYLITH_CHECK_ERROR(err);
    assert(!_viewer);
    err = VecDestroy(&_tstamp);
    PYLITH_CHECK_ERROR(err);
    assert(!_tstamp);
...
// ---------------------------------------------------------------------------------------------------------------------
// Copy constructor.
pylith::meshio::DataWriterHDF5::DataWriterHDF5(const DataWriterHDF5 &w) : DataWriter(w),
...
void pylith::meshio::DataWriterHDF5::open(const pylith::topology::Mesh &mesh,
                                          const bool isInfo)
{
    PYLITH_METHOD_BEGIN;
    ...
    try
    {
        ...
        err = MPI_Comm_rank(mesh.getComm(), &commRank);
        PYLITH_CHECK_ERROR(err);
        const int localSize = (!commRank) ? 1 : 0;
        err = VecCreateMPI(mesh.getComm(), localSize, 1, &_tstamp);
        PYLITH_CHECK_ERROR(err);
        assert(_tstamp);
        err = VecSetBlockSize(_tstamp, 1);
        PYLITH_CHECK_ERROR(err);
        err = PetscObjectSetName((PetscObject)_tstamp, "time");
        PYLITH_CHECK_ERROR(err);

        err = PetscViewerHDF5Open(mesh.getComm(), filename.c_str(), FILE_MODE_WRITE, &_viewer);
        PYLITH_CHECK_ERROR(err);
        err = PetscViewerHDF5SetBaseDimension2(_viewer, PETSC_TRUE);
        PYLITH_CHECK_ERROR(err);

        err = PetscViewerHDF5PushGroup(_viewer, "/geometry");
        PYLITH_CHECK_ERROR(err);
        ...
        if (isseq)
        {
            err = VecView_Seq(coordsGlobalVec, _viewer);
            PYLITH_CHECK_ERROR(err);
        }
        else
        {
            err = VecView_MPI(coordsGlobalVec, _viewer);
            PYLITH_CHECK_ERROR(err);
        } // if/else
        err = VecDestroy(&coordsGlobalVec);
        PYLITH_CHECK_ERROR(err);
        err = PetscViewerHDF5PopGroup(_viewer);
        PYLITH_CHECK_ERROR(err);
        ...
        err = MPI_Allreduce(&numCornersLocal, &numCorners, 1, MPIU_INT, MPI_MAX, mesh.getComm());
        PYLITH_CHECK_ERROR(err);
        ...
        PYLITH_CHECK_ERROR(err);
        err = VecCreate(mesh.getComm(), &cellVec);
        PYLITH_CHECK_ERROR(err);
        ...
        PYLITH_CHECK_ERROR(err);
        err = PetscViewerHDF5PushGroup(_viewer, "/topology");
        PYLITH_CHECK_ERROR(err);
        err = VecView(cellVec, _viewer);
        PYLITH_CHECK_ERROR(err);
        err = PetscViewerHDF5PopGroup(_viewer);
        PYLITH_CHECK_ERROR(err);
        ...
        hid_t h5 = -1;
        err = PetscViewerHDF5GetFileId(_viewer, &h5);
        PYLITH_CHECK_ERROR(err);
        assert(h5 >= 0);
...
// ---------------------------------------------------------------------------------------------------------------------
// Close output files.
void pylith::meshio::DataWriterHDF5::close(void)
{
    PYLITH_METHOD_BEGIN;
    ...
    if (isOpen())
    {
        ...
        PetscMPIInt commRank;
        err = MPI_Comm_rank(PETSC_COMM_WORLD, &commRank);
        PYLITH_CHECK_ERROR(err);
        if (!commRank)
        {
            try
...
void pylith::meshio::DataWriterHDF5::writeVertexField(const PylithScalar t,
                                                      const pylith::meshio::OutputSubfield &subfield)
{
    ...
    try
    {
        ...
        MPI_Comm comm;
        err = PetscObjectGetComm((PetscObject)subfield.getDM(), &comm);
        PYLITH_CHECK_ERROR(err);
        PetscMPIInt commRank;
        ...
        PYLITH_CHECK_ERROR(err);
        err = PetscViewerHDF5PushTimestepping(_viewer);
        PYLITH_CHECK_ERROR(err);
        err = PetscViewerHDF5SetTimestep(_viewer, istep);
        PYLITH_CHECK_ERROR(err);
        ...
        if (isseq)
        {
            err = VecView_Seq(vector, _viewer);
            PYLITH_CHECK_ERROR(err);
        }
        else
        {
            err = VecView_MPI(vector, _viewer);
            PYLITH_CHECK_ERROR(err);
        }
        err = PetscViewerHDF5PopTimestepping(_viewer);
        PYLITH_CHECK_ERROR(err);
        err = PetscViewerHDF5PopGroup(_viewer);
        PYLITH_CHECK_ERROR(err);
...
void pylith::meshio::DataWriterHDF5::writeCellField(const PylithScalar t,
                                                    const pylith::meshio::OutputSubfield &subfield)
{
    PYLITH_METHOD_BEGIN;
...
void pylith::meshio::DataWriterHDF5::writePointNames(const pylith::string_vector &names,
                                                     const pylith::topology::Mesh &mesh)
{
    ...
    try
    {
        ...
        hid_t h5 = -1;
        PetscErrorCode petscerr = PetscViewerHDF5GetFileId(_viewer, &h5);
        PYLITH_CHECK_ERROR(petscerr);
        assert(h5 >= 0);
        const char *parent = "/";
        ...
        if (err < 0)
        {
            throw std::runtime_error("Could not close filespace.");
        }
        ...
        if (memspace < 0)
        {
            throw std::runtime_error("Could not create memspace.");
        }
        ...
        if (err < 0)
        {
            throw std::runtime_error("Could not close memspace.");
        }
        ...
        delete[] namesFixedLength;
        namesFixedLength = NULL;
    }
    catch (const std::exception &err)
    {
        delete[] namesFixedLength;
...
void pylith::meshio::DataWriterHDF5::_writeTimeStamp(const PylithScalar t,
                                                     const int commRank)
{
    assert(_tstamp);
    PetscErrorCode err = 0;
    ...
    if (!commRank)
    {
        ...
        err = VecSetValue(_tstamp, 0, tDim, INSERT_VALUES);
        PYLITH_CHECK_ERROR(err);
    } // if
    err = VecAssemblyBegin(_tstamp);
    PYLITH_CHECK_ERROR(err);
    err = VecAssemblyEnd(_tstamp);
    PYLITH_CHECK_ERROR(err);

    err = PetscViewerHDF5PushGroup(_viewer, "/");
    PYLITH_CHECK_ERROR(err);
    err = PetscViewerHDF5PushTimestepping(_viewer);
    PYLITH_CHECK_ERROR(err);
    err = PetscViewerHDF5SetTimestep(_viewer, _tstampIndex);
    PYLITH_CHECK_ERROR(err);
    err = VecView(_tstamp, _viewer);
    PYLITH_CHECK_ERROR(err);
    err = PetscViewerHDF5PopTimestepping(_viewer);
    PYLITH_CHECK_ERROR(err);
    err = PetscViewerHDF5PopGroup(_viewer);
