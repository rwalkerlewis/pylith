...
// ----------------------------------------------------------------------
// Constructor
pylith::meshio::MeshIO::MeshIO(void) : _mesh(0),
                                       _debug(false)
{ // constructor
} // constructor
...
void pylith::meshio::MeshIO::deallocate(void)
{ // deallocate
} // deallocate

// ----------------------------------------------------------------------
// Get spatial dimension of mesh.
int pylith::meshio::MeshIO::getMeshDim(void) const
{ // getMeshDim
...
// ----------------------------------------------------------------------
// Read mesh from file.
void pylith::meshio::MeshIO::read(topology::Mesh *mesh)
{
    PYLITH_METHOD_BEGIN;
    ...
    err = DMViewFromOptions(_mesh->getDM(), NULL, "-pylith_dm_view");
...
// ----------------------------------------------------------------------
// Write mesh to file.
void pylith::meshio::MeshIO::write(topology::Mesh *const mesh)
{ // write
    PYLITH_METHOD_BEGIN;
...
void pylith::meshio::MeshIO::_getVertices(scalar_array *coordinates,
                                          int *numVertices,
                                          int *spaceDim) const
{ // _getVertices
    PYLITH_METHOD_BEGIN;
    ...
    *spaceDim = cs->getSpaceDim();
    ...
    err = DMGetCoordinatesLocal(dmMesh, &coordVec);
    PYLITH_CHECK_ERROR(err);
    err = VecGetArray(coordVec, &coordArray);
    PYLITH_CHECK_ERROR(err);
    err = VecGetLocalSize(coordVec, &coordSize);
    PYLITH_CHECK_ERROR(err);
    ...
    *numVertices = coordSize / *spaceDim;
    ...
    err = VecRestoreArray(coordVec, &coordArray);
    PYLITH_CHECK_ERROR(err);
...
// ----------------------------------------------------------------------
// Get cells in mesh.
void pylith::meshio::MeshIO::_getCells(int_array *cells,
                                       int *numCells,
                                       int *numCorners,
                                       int *meshDim) const
{
    ...
    *numCells = pylith::topology::MeshOps::getNumCells(*_mesh);
    assert(*numCells > 0);
    *numCorners = pylith::topology::MeshOps::getNumCorners(*_mesh);
    assert(*numCorners > 0);
    *meshDim = _mesh->getDimension();
    ...
    for (PetscInt c = cStart, index = 0; c < cEnd; ++c)
    {
        ...
        err = DMPlexGetCellType(dmMesh, c, &ct);
        PYLITH_CHECK_ERROR(err);
        err = DMPlexGetTransitiveClosure(dmMesh, c, PETSC_TRUE, &closureSize, &closure);
        PYLITH_CHECK_ERROR(err);
        ...
        err = DMPlexRestoreTransitiveClosure(dmMesh, c, PETSC_TRUE, &closureSize, &closure);
        PYLITH_CHECK_ERROR(err);
        err = DMPlexInvertCell(ct, &(*cells)[index - nC]);
        PYLITH_CHECK_ERROR(err);
...
void pylith::meshio::MeshIO::_setMaterials(const int_array &materialIds)
{
    PYLITH_METHOD_BEGIN;
    assert(_mesh);
    ...
    if (!_mesh->getCommRank())
    {
        ...
        {
            err = DMSetLabelValue(dmMesh, labelName, c, materialIds[c - cStart]);
            PYLITH_CHECK_ERROR(err);
        } // for
    }
    else
    {
        err = DMCreateLabel(dmMesh, labelName);
        PYLITH_CHECK_ERROR(err);
    } // if/else
...
void pylith::meshio::MeshIO::_getMaterials(int_array *materialIds) const
{
    PYLITH_METHOD_BEGIN;
    ...
    for (PetscInt c = cStart, index = 0; c < cEnd; ++c)
    {
        err = DMGetLabelValue(dmMesh, labelName, c, &matId);
        PYLITH_CHECK_ERROR(err);
...
void pylith::meshio::MeshIO::_setGroup(const std::string &name,
...
{ // _setGroup
    ...
    err = DMCreateLabel(dmMesh, name.c_str());
    PYLITH_CHECK_ERROR(err);
    err = DMGetLabel(dmMesh, name.c_str(), &label);
    PYLITH_CHECK_ERROR(err);
    if (CELL == type)
    {
        for (PetscInt p = 0; p < numPoints; ++p)
        {
            err = DMLabelSetValue(label, points[p], 1);
            PYLITH_CHECK_ERROR(err);
        } // for
    }
    else if (VERTEX == type)
    {
        ...
        err = DMPlexGetHeightStratum(dmMesh, 0, &cStart, &cEnd);
        PYLITH_CHECK_ERROR(err);
        ...
        for (PetscInt p = 0; p < numPoints; ++p)
        {
            err = DMLabelSetValue(label, numCells + points[p], 1);
            PYLITH_CHECK_ERROR(err);
        } // for
        ...
        for (PetscInt p = 0; p < numPoints; ++p)
        {
            ...
            err = DMPlexGetTransitiveClosure(dmMesh, vertex, PETSC_FALSE, &starSize, &star);
            PYLITH_CHECK_ERROR(err);
            for (s = 0; s < starSize * 2; s += 2)
            {
                ...
                err = DMPlexGetTransitiveClosure(dmMesh, point, PETSC_TRUE, &closureSize, &closure);
                PYLITH_CHECK_ERROR(err);
                for (c = 0; c < closureSize * 2; c += 2)
                {
                    if ((closure[c] >= vStart) && (closure[c] < vEnd))
                    {
                        err = DMLabelGetValue(label, closure[c], &value);
                        PYLITH_CHECK_ERROR(err);
                        if (value != 1)
                        {
                            marked = PETSC_FALSE;
                ...
                err = DMPlexRestoreTransitiveClosure(dmMesh, point, PETSC_TRUE, &closureSize, &closure);
                PYLITH_CHECK_ERROR(err);
                if (marked)
                {
                    err = DMLabelSetValue(label, point, 1);
                    PYLITH_CHECK_ERROR(err);
                }
            }
            err = DMPlexRestoreTransitiveClosure(dmMesh, vertex, PETSC_FALSE, &starSize, &star);
            PYLITH_CHECK_ERROR(err);
        }
    } // if/else
...
void pylith::meshio::MeshIO::_distributeGroups()
{ // _distributeGroups
    PYLITH_METHOD_BEGIN;
...
void pylith::meshio::MeshIO::_getGroupNames(string_vector *names) const
{ // _getGroups
    PYLITH_METHOD_BEGIN;
    ...
    PetscErrorCode err = 0;
    err = DMGetNumLabels(dmMesh, &numLabels);
    PYLITH_CHECK_ERROR(err);
    ...
    for (int iGroup = 0, iLabel = 0; iLabel < numLabels; ++iLabel)
    {
        ...
        err = DMGetLabelName(dmMesh, iLabel, &labelName);
        PYLITH_CHECK_ERROR(err);
        if ((std::string(labelName) != std::string("depth")) && (std::string(labelName) != std::string("celltype")) && (std::string(labelName) != materialLabelName))
        {
...
// ----------------------------------------------------------------------
// Get group entities
void pylith::meshio::MeshIO::_getGroup(int_array *points,
...
{ // _getGroup
    ...
    PetscErrorCode err;
    err = DMGetStratumIS(dmMesh, name, 1, &groupIS);
    PYLITH_CHECK_ERROR(err);
    err = ISGetIndices(groupIS, &groupIndices);
    PYLITH_CHECK_ERROR(err);

    PetscInt totalSize;
    err = DMGetStratumSize(dmMesh, name, 1, &totalSize);
    PYLITH_CHECK_ERROR(err);
    ...
    *groupType = VERTEX;
    ...
    err = ISRestoreIndices(groupIS, &groupIndices);
    PYLITH_CHECK_ERROR(err);
    err = ISDestroy(&groupIS);
    PYLITH_CHECK_ERROR(err);
