...
// ------------------------------------------------------------------------------------------------
// Constructor
pylith::problems::GreensFns::GreensFns(void) : _faultLabelName(pylith::topology::Mesh::cells_label_name),
...
void pylith::problems::GreensFns::deallocate(void)
{
    PYLITH_METHOD_BEGIN;
    ...
    PetscErrorCode err = SNESDestroy(&_snes);
...
void pylith::problems::GreensFns::setFaultLabelName(const char *value)
{
...
void pylith::problems::GreensFns::setFaultLabelValue(const int value)
{
...
int pylith::problems::GreensFns::getFaultLabelValue(void) const
{
...
void pylith::problems::GreensFns::setProgressMonitor(pylith::problems::ProgressMonitorStep *monitor)
{
...
pylith::problems::GreensFns::getPetscDM(void)
{
    ...
    PetscDM dm = NULL;
    PetscErrorCode err = SNESGetDM(_snes, &dm);
    PYLITH_CHECK_ERROR(err);
...
void pylith::problems::GreensFns::verifyConfiguration(void) const
{
    PYLITH_METHOD_BEGIN;
...
// ------------------------------------------------------------------------------------------------
// Initialize.
void pylith::problems::GreensFns::initialize(void)
{
    PYLITH_METHOD_BEGIN;
    ...
    err = SNESCreate(solution->getMesh().getComm(), &_snes);
    PYLITH_CHECK_ERROR(err);
    assert(_snes);
    err = SNESSetApplicationContext(_snes, (void *)this);
    PYLITH_CHECK_ERROR(err);
    err = SNESSetDM(_snes, solution->getDM());
    PYLITH_CHECK_ERROR(err);

    PetscVec solutionVector = solution->getGlobalVector();
    solution->scatterLocalToVector(solutionVector);
    err = SNESSetSolution(_snes, solutionVector);
    PYLITH_CHECK_ERROR(err);

    // Initialize solution_dot.
    pylith::topology::Field *solutionDot = new pylith::topology::Field(*solution);
    assert(solutionDot);
    ...
    case pylith::problems::Physics::QUASISTATIC:
        ...
        err = SNESSetFunction(_snes, NULL, computeResidual, (void *)this);
        PYLITH_CHECK_ERROR(err);
        err = SNESSetJacobian(_snes, NULL, NULL, computeJacobian, (void *)this);
        PYLITH_CHECK_ERROR(err);
        err = SNESSetType(_snes, SNESKSPONLY);
        PYLITH_CHECK_ERROR(err);
        break;
    ...
    {
        PetscDS dsSoln = NULL;
        err = DMGetDS(solution->getDM(), &dsSoln);
        PYLITH_CHECK_ERROR(err);
...
void pylith::problems::GreensFns::solve(void)
{
    PYLITH_METHOD_BEGIN;
    ...
    for (size_t i = 0; i < numImpulses; ++i)
    {
        ...
        PetscErrorCode err = SNESSolve(_snes, residual->getGlobalVector(), solution->getGlobalVector());
        PYLITH_CHECK_ERROR(err);
...
void pylith::problems::GreensFns::poststep(const size_t impulse)
{
    PYLITH_METHOD_BEGIN;
...
void pylith::problems::GreensFns::setSolutionLocal(PetscVec solutionVec)
{
    PYLITH_METHOD_BEGIN;
...
// ------------------------------------------------------------------------------------------------
// Compute residual.
void pylith::problems::GreensFns::computeResidual(PetscVec residualVec,
...
// ------------------------------------------------------------------------------------------------
// Compute Jacobian.
void pylith::problems::GreensFns::computeJacobian(PetscMat jacobianMat,
                                                  PetscMat precondMat,
                                                  PetscVec solutionVec)
{
    PYLITH_METHOD_BEGIN;
    ...
    err = PetscDSHasJacobian(dsSoln, &hasJacobian);
    PYLITH_CHECK_ERROR(err);
    if (hasJacobian)
    {
    ...
    err = PetscDSHasJacobianPreconditioner(dsSoln, &hasPreconditioner);
    PYLITH_CHECK_ERROR(err);
                                             ...
                                             void *context)
{
    ...
    pythia::journal::debug_t debug(_GreensFns::pyreComponent);
    debug << pythia::journal::at(__HERE__)
          << "computeJacobian(snes=" << snes << ", solutionVec=" << solutionVec << ", jacobianMat=" << jacobianMat << ", precondMat=" << precondMat << ", context=" << context << ")" << pythia::journal::endl;
