...
// ------------------------------------------------------------------------------------------------
// Constructor
pylith::problems::InitialConditionPatch::InitialConditionPatch(void) : _labelName(pylith::topology::Mesh::cells_label_name),
                                                                       _labelValue(1),
                                                                       _db(NULL)
{
...
void pylith::problems::InitialConditionPatch::deallocate(void)
{
    PYLITH_METHOD_BEGIN;
...
void pylith::problems::InitialConditionPatch::setLabelName(const char *value)
{
...
void pylith::problems::InitialConditionPatch::setLabelValue(const int value)
{
    _labelValue = value;
} // setLabelValue
...
int pylith::problems::InitialConditionPatch::getLabelValue(void) const
{
    return _labelValue;
} // getLabelValue
...
void pylith::problems::InitialConditionPatch::setDB(spatialdata::spatialdb::SpatialDB *db)
{
    PYLITH_METHOD_BEGIN;
...
void pylith::problems::InitialConditionPatch::verifyConfiguration(const pylith::topology::Field &solution) const
{
    PYLITH_METHOD_BEGIN;
    ...
    PetscErrorCode err = DMHasLabel(dmSoln, _labelName.c_str(), &hasLabel);
    ...
    err = DMGetLabel(solution.getDM(), _labelName.c_str(), &dmLabel);
    PYLITH_CHECK_ERROR(err);
    assert(dmLabel);
    PetscBool hasValue = PETSC_FALSE;
    err = DMLabelHasValue(dmLabel, _labelValue, &hasValue);
    PYLITH_CHECK_ERROR(err);
    if (!hasValue)
    {
        std::ostringstream msg;
...
// ------------------------------------------------------------------------------------------------
// Set solver type.
void pylith::problems::InitialConditionPatch::setValues(pylith::topology::Field *solution,
