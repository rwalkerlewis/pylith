...
// ---------------------------------------------------------------------------------------------------------------------
// Constructor
pylith::problems::TimeDependent::TimeDependent(void) : _startTime(0.0),
                                                       _endTime(0.0),
                                                       _dtInitial(1.0),
                                                       _maxTimeSteps(0),
                                                       _ts(NULL),
...
void pylith::problems::TimeDependent::deallocate(void)
{
    PYLITH_METHOD_BEGIN;
    ...
    PetscErrorCode err = TSDestroy(&_ts);
...
void pylith::problems::TimeDependent::setStartTime(const double value)
{
...
void pylith::problems::TimeDependent::setEndTime(const double value)
{
    PYLITH_METHOD_BEGIN;
...
void pylith::problems::TimeDependent::setMaxTimeSteps(const size_t value)
{
    PYLITH_METHOD_BEGIN;
...
void pylith::problems::TimeDependent::setInitialTimeStep(const double value)
{
    PYLITH_METHOD_BEGIN;
...
void pylith::problems::TimeDependent::setInitialCondition(pylith::problems::InitialCondition *ic[],
                                                          const int numIC)
{
    PYLITH_METHOD_BEGIN;
...
void pylith::problems::TimeDependent::setShouldNotifyIC(const bool value)
{
...
void pylith::problems::TimeDependent::setProgressMonitor(pylith::problems::ProgressMonitorTime *monitor)
{
...
pylith::problems::TimeDependent::getPetscSNES(void)
{
    ...
    PetscSNES snes = NULL;
    PetscErrorCode err = TSGetSNES(_ts, &snes);
    PYLITH_CHECK_ERROR(err);
...
void pylith::problems::TimeDependent::verifyConfiguration(void) const
{
    PYLITH_METHOD_BEGIN;
...
// ---------------------------------------------------------------------------------------------------------------------
// Initialize.
void pylith::problems::TimeDependent::initialize(void)
{
    PYLITH_METHOD_BEGIN;
    ...
    err = TSCreate(mesh.getComm(), &_ts);
    PYLITH_CHECK_ERROR(err);
    assert(_ts);
    err = TSSetType(_ts, TSBEULER);
    PYLITH_CHECK_ERROR(err); // Backward Euler is default time stepping method.
    err = TSSetExactFinalTime(_ts, TS_EXACTFINALTIME_STEPOVER);
    PYLITH_CHECK_ERROR(err); // Ok to step over final time.
    err = TSSetApplicationContext(_ts, (void *)this);
    PYLITH_CHECK_ERROR(err);
    ...
    case LINEAR:
        ...
        err = TSSetProblemType(_ts, TS_LINEAR);
        PYLITH_CHECK_ERROR(err);
        break;
    case NONLINEAR:
        PYLITH_COMPONENT_DEBUG("Setting PetscTS problem type to 'nonlinear'.");
        err = TSSetProblemType(_ts, TS_NONLINEAR);
        PYLITH_CHECK_ERROR(err);
        break;
    default:
    ...
    err = TSSetTime(_ts, _startTime / timeScale);
    PYLITH_CHECK_ERROR(err);
    err = TSSetTimeStep(_ts, _dtInitial / timeScale);
    PYLITH_CHECK_ERROR(err);
    err = TSSetMaxSteps(_ts, _maxTimeSteps);
    PYLITH_CHECK_ERROR(err);
    err = TSSetMaxTime(_ts, _endTime / timeScale);
    PYLITH_CHECK_ERROR(err);
    err = TSSetDM(_ts, solution->getDM());
    PYLITH_CHECK_ERROR(err);
    ...
    err = TSSetSolution(_ts, solutionVector);
    PYLITH_CHECK_ERROR(err);
    assert(_observers);
    _observers->setTimeScale(timeScale);

    // Initialize residual.
    pylith::topology::Field *residual = new pylith::topology::Field(*solution);
    assert(residual);
    ...
    err = TSSetPostStep(_ts, poststep);
    PYLITH_CHECK_ERROR(err);
    ...
    case pylith::problems::Physics::QUASISTATIC:
        ...
        err = TSSetIFunction(_ts, NULL, computeLHSResidual, (void *)this);
        PYLITH_CHECK_ERROR(err);
        err = TSSetIJacobian(_ts, NULL, NULL, computeLHSJacobian, (void *)this);
        PYLITH_CHECK_ERROR(err);
        break;
    case pylith::problems::Physics::DYNAMIC_IMEX:
        ...
        err = TSSetIJacobian(_ts, NULL, NULL, computeLHSJacobian, (void *)this);
        PYLITH_CHECK_ERROR(err);
        err = TSSetEquationType(_ts, TS_EQ_EXPLICIT);
        PYLITH_CHECK_ERROR(err);
    case pylith::problems::Physics::DYNAMIC:
    {
        PYLITH_COMPONENT_DEBUG("Setting PetscTS callback for computeRHSFunction().");
        err = TSSetRHSFunction(_ts, NULL, computeRHSResidual, (void *)this);
        PYLITH_CHECK_ERROR(err);
        PYLITH_COMPONENT_DEBUG("Setting up field for inverse of lumped LHS Jacobian.");
        pylith::topology::Field *jacobianLHSLumpedInv = new pylith::topology::Field(*solution);
        assert(jacobianLHSLumpedInv);
    ...
    {
        PetscDS prob = NULL;
        err = DMGetDS(solution->getDM(), &prob);
        PYLITH_CHECK_ERROR(err);
...
static PetscErrorCode quad_linear_p(PetscInt dim, PetscReal time, const PetscReal x[], PetscInt Nc, PetscScalar *u, void *ctx)
{
    u[0] = (x[1] * x[1] - 1) * time;
    return 0;
}
...
void pylith::problems::TimeDependent::poststep(void)
{
    PYLITH_METHOD_BEGIN;
    ...
    err = TSGetTime(_ts, &t);
    PYLITH_CHECK_ERROR(err);
    err = TSGetTimeStep(_ts, &dt);
    PYLITH_CHECK_ERROR(err);
    err = TSGetStepNumber(_ts, &tindex);
    PYLITH_CHECK_ERROR(err);
    err = TSGetSolution(_ts, &solutionVec);
    PYLITH_CHECK_ERROR(err);
...
void pylith::problems::TimeDependent::setSolutionLocal(const PylithReal t,
                                                       ...
                                                       PetscVec solutionDotVec)
{
    ...
    if (solutionDotVec)
    {
        ...
        {
            solutionDot = new pylith::topology::Field(*solution);
            assert(solutionDot);
...
void pylith::problems::TimeDependent::computeRHSResidual(PetscVec residualVec,
                                                         const PylithReal t,
                                                         const PylithReal dt,
                                                         PetscVec solutionVec)
{
    ...
    if (t != _integrationData->getScalar(pylith::feassemble::IntegrationData::t_state))
    {
        _setState(t);
    }
    ...
    PetscErrorCode err = VecSet(residualVec, 0.0);
    PYLITH_CHECK_ERROR(err);
...
void pylith::problems::TimeDependent::computeLHSResidual(PetscVec residualVec,
                                                         const PylithReal t,
                                                         const PylithReal dt,
                                                         PetscVec solutionVec,
                                                         PetscVec solutionDotVec)
...
void pylith::problems::TimeDependent::computeLHSJacobian(PetscMat jacobianMat,
                                                         PetscMat precondMat,
                                                         const PylithReal t,
                                                         const PylithReal dt,
                                                         ...
                                                         PetscVec solutionDotVec)
{
    ...
    err = DMGetDS(solution->getDM(), &solnDS);
    PYLITH_CHECK_ERROR(err);
    err = PetscDSHasJacobian(solnDS, &hasJacobian);
    PYLITH_CHECK_ERROR(err);
    if (hasJacobian)
    {
        err = MatZeroEntries(jacobianMat);
        PYLITH_CHECK_ERROR(err);
    }
    err = MatZeroEntries(precondMat);
    PYLITH_CHECK_ERROR(err);
...
void pylith::problems::TimeDependent::computeLHSJacobianLumpedInv(const PylithReal t,
                                                                  const PylithReal dt,
                                                                  const PylithReal s_tshift,
                                                                  PetscVec solutionVec)
{
                                                    ...
                                                    void *context)
{
    ...
    if (hasLumpedJacobianInverse)
    {
        ...
        const pylith::topology::Field *jacobianLumpedInv =
            problem->_integrationData->getField(pylith::feassemble::IntegrationData::lumped_jacobian_inverse);
        assert(jacobianLumpedInv);
        err = VecPointwiseMult(residualVec, jacobianLumpedInv->getGlobalVector(), residualVec);
        PYLITH_CHECK_ERROR(err);
    } // if
                                                    ...
                                                    void *context)
{
    ...
    pythia::journal::debug_t debug(_TimeDependent::pyreComponent);
    debug << pythia::journal::at(__HERE__)
          << "computeLHSJacobian(ts=" << ts << ", t=" << t << ", solutionVec=" << solutionVec << ", solutionDotVec=" << solutionDotVec << ", s_tshift=" << s_tshift << ", jacobianMat=" << jacobianMat << ", precondMat=" << precondMat << ", context=" << context << ")" << pythia::journal::endl;
...
pylith::problems::TimeDependent::poststep(PetscTS ts)
{
    ...
    PetscErrorCode err = TSGetApplicationContext(ts, (void *)&problem);
    PYLITH_CHECK_ERROR(err);
    assert(problem);
    problem->poststep();
...
bool pylith::problems::TimeDependent::_needNewJacobian(const PylithReal dt)
{
    PYLITH_METHOD_BEGIN;

    // If we already know we need to recompute the LHS Jacobian, then return true.
    if (_needNewLHSJacobian)
    {
...
void pylith::problems::TimeDependent::_setState(const PylithReal t)
{
    PYLITH_METHOD_BEGIN;
...
void pylith::problems::TimeDependent::_notifyObserversInitialSoln(void)
{
    PYLITH_METHOD_BEGIN;
