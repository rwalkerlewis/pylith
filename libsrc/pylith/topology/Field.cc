...
// ------------------------------------------------------------------------------------------------
// Default constructor.
pylith::topology::Field::Field(const pylith::topology::Mesh &mesh) : _mesh(NULL),
                                                                     _localVec(NULL),
                                                                     _globalVec(NULL),
                                                                     _outputVec(NULL)
{
...
pylith::topology::Field::Field(const Field &src) : _mesh(NULL),
                                                   _localVec(NULL),
                                                   _globalVec(NULL),
                                                   _outputVec(NULL)
{
    ...
    PetscErrorCode err;
    assert(_mesh);
    err = DMCopyDisc(src._mesh->getDM(), _mesh->getDM());
    PYLITH_CHECK_ERROR(err);

    assert(!_localVec);
    err = DMCreateLocalVector(_mesh->getDM(), &_localVec);
    PYLITH_CHECK_ERROR(err);
    err = VecSet(_localVec, 0.0);
    PYLITH_CHECK_ERROR(err);
...
void pylith::topology::Field::deallocate(void)
{
    PYLITH_METHOD_BEGIN;
    ...
    PetscErrorCode err;
    err = VecDestroy(&_localVec);
    PYLITH_CHECK_ERROR(err);
    err = VecDestroy(&_globalVec);
    PYLITH_CHECK_ERROR(err);
    err = VecDestroy(&_outputVec);
    PYLITH_CHECK_ERROR(err);
...
// ------------------------------------------------------------------------------------------------
// Set label for field.
void pylith::topology::Field::setLabel(const char *value)
{
    PYLITH_METHOD_BEGIN;
    assert(_mesh);
    ...
    if (_mesh->getDM())
    {
        err = PetscObjectSetName((PetscObject)_mesh->getDM(), value);
        PYLITH_CHECK_ERROR(err);
    } // of
    if (_localVec)
    {
        err = PetscObjectSetName((PetscObject)_localVec, value);
        PYLITH_CHECK_ERROR(err);
    } // if
    if (_globalVec)
    {
        err = PetscObjectSetName((PetscObject)_globalVec, value);
        PYLITH_CHECK_ERROR(err);
    } // if
    if (_outputVec)
    {
        err = PetscObjectSetName((PetscObject)_outputVec, value);
        PYLITH_CHECK_ERROR(err);
    } // if
...
pylith::topology::Field::getLocalSection(void) const
{
    ...
    PetscSection s = NULL;
    PetscErrorCode err = DMGetSection(_mesh->getDM(), &s);
    PYLITH_CHECK_ERROR(err);
...
pylith::topology::Field::getGlobalSection(void) const
{
    ...
    PetscSection s = NULL;
    PetscErrorCode err = DMGetGlobalSection(_mesh->getDM(), &s);
    PYLITH_CHECK_ERROR(err);
    PYLITH_METHOD_RETURN(s);
}
...
pylith::topology::Field::getChartSize(void) const
{
    ...
    err = DMGetSection(_mesh->getDM(), &s);
    PYLITH_CHECK_ERROR(err);
    err = PetscSectionGetChart(s, &pStart, &pEnd);
    PYLITH_CHECK_ERROR(err);
...
pylith::topology::Field::getStorageSize(void) const
{
    ...
    err = DMGetSection(_mesh->getDM(), &s);
    PYLITH_CHECK_ERROR(err);
    err = PetscSectionGetStorageSize(s, &size);
    PYLITH_CHECK_ERROR(err);
...
// ------------------------------------------------------------------------------------------------
void pylith::topology::Field::createDiscretization(void)
{
    PYLITH_METHOD_BEGIN;
    assert(_mesh);

    PetscErrorCode err = DMCreateDS(_mesh->getDM());
    PYLITH_CHECK_ERROR(err);
...
void pylith::topology::Field::allocate(void)
{
    PYLITH_METHOD_BEGIN;
    assert(_mesh);
    ...
    assert(dm);
    err = DMGetSection(dm, &s);
    PYLITH_CHECK_ERROR(err);
    assert(s); // Creates local section
    err = DMSetGlobalSection(dm, NULL);
    PYLITH_CHECK_ERROR(err); // Creates global section
    err = PetscSectionSetUp(s);
    PYLITH_CHECK_ERROR(err);

    err = VecDestroy(&_localVec);
    PYLITH_CHECK_ERROR(err);
    err = DMCreateLocalVector(dm, &_localVec);
    PYLITH_CHECK_ERROR(err);
    err = PetscObjectSetName((PetscObject)_localVec, _label.c_str());
    PYLITH_CHECK_ERROR(err);
    err = VecSet(_localVec, 0.0);
    PYLITH_CHECK_ERROR(err);
...
void pylith::topology::Field::zeroLocal(void)
{
    PYLITH_METHOD_BEGIN;

    assert(_localVec);
    PetscErrorCode err = VecSet(_localVec, 0.0);
    PYLITH_CHECK_ERROR(err);
...
void pylith::topology::Field::view(const char *label,
                                   const ViewOptions options) const
{
    ...
    {
        err = DMView(dm, PETSC_VIEWER_STDOUT_WORLD);
        PYLITH_CHECK_ERROR(err);
        err = PetscSectionView(section, PETSC_VIEWER_STDOUT_WORLD);
        PYLITH_CHECK_ERROR(err);
    } // if
    if ((VIEW_VALUES == options) || (VIEW_ALL == options))
    {
        err = MPI_Comm_size(PetscObjectComm((PetscObject)dm), &numProcs);
        PYLITH_CHECK_ERROR(err);
        err = MPI_Comm_rank(PetscObjectComm((PetscObject)dm), &rank);
        PYLITH_CHECK_ERROR(err);
        for (PetscInt p = 0; p < numProcs; ++p)
        {
            ...
            if (p == rank)
            {
                err = VecView(_localVec, PETSC_VIEWER_STDOUT_SELF);
                PYLITH_CHECK_ERROR(err);
            }
            err = PetscBarrier((PetscObject)dm);
            PYLITH_CHECK_ERROR(err);
        } // for
    }     // if
...
void pylith::topology::Field::scatterLocalToVector(const PetscVec vector,
                                                   InsertMode mode) const
{
    ...
    PetscErrorCode err;
    assert(_localVec);
    err = DMLocalToGlobalBegin(_mesh->getDM(), _localVec, mode, vector);
    PYLITH_CHECK_ERROR(err);
    err = DMLocalToGlobalEnd(_mesh->getDM(), _localVec, mode, vector);
    PYLITH_CHECK_ERROR(err);
...
void pylith::topology::Field::scatterVectorToLocal(const PetscVec vector,
                                                   InsertMode mode) const
{
    PYLITH_METHOD_BEGIN;
    assert(_mesh);
    ...
    PetscErrorCode err;
    assert(_localVec);
    err = DMGlobalToLocalBegin(_mesh->getDM(), vector, mode, _localVec);
    PYLITH_CHECK_ERROR(err);
    err = DMGlobalToLocalEnd(_mesh->getDM(), vector, mode, _localVec);
    PYLITH_CHECK_ERROR(err);
...
void pylith::topology::Field::scatterLocalToOutput(InsertMode mode) const
{
    PYLITH_METHOD_BEGIN;
    assert(_mesh);
    ...
    err = DMGetOutputDM(_mesh->getDM(), &dmOutput);
    PYLITH_CHECK_ERROR(err);
    if (dmOutput)
    {
        assert(_localVec);
        err = DMLocalToGlobalBegin(dmOutput, _localVec, mode, _outputVec);
        PYLITH_CHECK_ERROR(err);
        err = DMLocalToGlobalEnd(dmOutput, _localVec, mode, _outputVec);
        PYLITH_CHECK_ERROR(err);
    } // if
...
void pylith::topology::Field::subfieldAdd(const char *name,
                                          const char *alias,
...
// ------------------------------------------------------------------------------------------------
// Add subfield.
void pylith::topology::Field::subfieldAdd(const Description &description,
...
// ------------------------------------------------------------------------------------------------
void pylith::topology::Field::subfieldsSetup(void)
{
    PYLITH_METHOD_BEGIN;
    assert(_mesh);
    ...
    for (subfields_type::iterator s_iter = _subfields.begin(); s_iter != _subfields.end(); ++s_iter)
    {
        ...
        PetscFE fe = FieldOps::createFE(sinfo.fe, dm, sinfo.description.numComponents);
        assert(fe);
        err = PetscFESetName(fe, sname);
        PYLITH_CHECK_ERROR(err);
...
bool pylith::topology::Field::hasSubfield(const char *name) const
{
    PYLITH_METHOD_BEGIN;
...
void pylith::topology::Field::createGlobalVector(void)
{
    PYLITH_METHOD_BEGIN;
    assert(_mesh);

    PetscErrorCode err = VecDestroy(&_globalVec);
    PYLITH_CHECK_ERROR(err);
    err = DMCreateGlobalVector(_mesh->getDM(), &_globalVec);
    PYLITH_CHECK_ERROR(err);
    assert(_globalVec);
    err = PetscObjectSetName((PetscObject)_globalVec, getLabel());
    PYLITH_CHECK_ERROR(err);
...
void pylith::topology::Field::createOutputVector(void)
{
    PYLITH_METHOD_BEGIN;
    assert(_mesh);

    PetscErrorCode err = VecDestroy(&_outputVec);
    PYLITH_CHECK_ERROR(err);
    ...
    err = DMGetDS(dmOutput, &dsOutput);
    PYLITH_CHECK_ERROR(err);
    err = PetscDSSetUp(dsOutput);
    PYLITH_CHECK_ERROR(err);

    err = DMCreateGlobalVector(dmOutput, &_outputVec);
    PYLITH_CHECK_ERROR(err);
    assert(_outputVec);
    err = PetscObjectSetName((PetscObject)_outputVec, getLabel());
    PYLITH_CHECK_ERROR(err);
