...
For each fault, which is an internal interface, we add a boundary condition to the thermoelasticity equation prescribing the jump in the displacement field across the fault,
%
```{math}
:label: eqn:bc:prescribed:slip:thermoelasticity
\begin{gathered}
...
We apply conservation of momentum,
\begin{equation}
...
In thermoelasticity, fault slip can generate heat through frictional dissipation.
The frictional heating rate per unit area on the fault is given by
\begin{equation}
...
This heating term can be incorporated into the energy balance equation as a volumetric heat source in a thin layer adjacent to the fault, or as a boundary condition on the fault surface.

For earthquake rupture problems, this frictional heating can significantly affect the temperature field near the fault, potentially weakening the fault and influencing subsequent rupture behavior.

```{table} Mathematical notation for thermoelasticity equation with infinitesimal strain and prescribed slip on faults.
:name: tab:notation:thermoelasticity:prescribed:slip
