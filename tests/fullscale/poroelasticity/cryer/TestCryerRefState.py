...
class TestCase(FullTestCase):
    ...
    def setUp(self):
        defaults = {
            ...
            "exact_soln": cryer_refstate_soln.AnalyticalSoln(),
            "mesh": self.mesh,
        }
        self.checks = [
            Check(
...
class TestHex(TestCase):
    ...
    def setUp(self):
        self.name = "cryer_refstate_hex"
        self.mesh = meshes.Hex()
        super().setUp()

        TestCase.run_pylith(self, self.name, ["cryer_refstate.cfg", "cryer_refstate_hex.cfg"])
        return
...
class TestTet(TestCase):
    ...
    def setUp(self):
        self.name = "cryer_refstate_tet"
        self.mesh = meshes.Tet()
        super().setUp()

        TestCase.run_pylith(self, self.name, ["cryer_refstate.cfg", "cryer_refstate_tet.cfg"])
        return
