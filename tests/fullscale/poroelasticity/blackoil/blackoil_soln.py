# =================================================================================================
# This code is part of PyLith, developed through the Computational Infrastructure
# for Geodynamics (https://github.com/geodynamics/pylith).
#
# Copyright (c) 2010-2025, University of California, Davis and the PyLith Development Team.
# All rights reserved.
#
# See https://mit-license.org/ and LICENSE.md and for license information.
# =================================================================================================
# @brief Analytical solution for black oil poroelasticity pressure diffusion test.
#
# This test verifies the black oil formulation with pressure-dependent fluid properties.
# We use a steady-state pressure gradient problem where the solution can be computed analytically.

import numpy


# Physical properties
p_solid_density = 2500.0
p_fluid_density = 1000.0
p_fluid_viscosity = 1.0e-3  # Reference viscosity at p_ref
p_porosity = 0.02

p_shear_modulus = 3.0e10
p_drained_bulk_modulus = 8.0e10
p_fluid_bulk_modulus = 1.0e10
p_biot_coefficient = 0.7
p_isotropic_permeability = 1.0e-14

# Black oil specific parameters
p_reference_pressure = 1.0e7  # 10 MPa
p_fluid_compressibility = 1.0e-10  # Reference compressibility (1/Pa)
p_fluid_compressibility_coefficient = 0.0  # No pressure dependence for analytical solution
p_viscosity_coefficient = 0.0  # No pressure dependence for analytical solution


p_mu = p_shear_modulus
p_lambda = p_drained_bulk_modulus - 2.0 / 3.0 * p_shear_modulus


x_min = -4.0e3
x_max = +4.0e3
fx = 10.0e3  # kPa/m


# ----------------------------------------------------------------------
class AnalyticalSoln(object):
    """Analytical solution for black oil poroelasticity problem.
    
    For this test, we use zero pressure-dependence coefficients
    (viscosity_coefficient=0, fluid_compressibility_coefficient=0)
    so the solution reduces to the standard poroelasticity case.
    This allows us to validate that the black oil implementation
    correctly reduces to standard poroelasticity in this limit.
    """

    SPACE_DIM = 2
    TENSOR_SIZE = 4

    def __init__(self):
        self.fields = {
            "displacement": self.displacement,
            "pressure": self.pressure,
            "trace_strain": self.trace_strain,
            "solid_density": self.solid_density,
            "fluid_density": self.fluid_density,
            "fluid_viscosity": self.fluid_viscosity,
            "shear_modulus": self.shear_modulus,
            "drained_bulk_modulus": self.drained_bulk_modulus,
            "biot_coefficient": self.biot_coefficient,
            "biot_modulus": self.biot_modulus,
            "isotropic_permeability": self.isotropic_permeability,
            "porosity": self.porosity,
            "reference_pressure": self.reference_pressure,
            "fluid_compressibility": self.fluid_compressibility,
            "fluid_compressibility_coefficient": self.fluid_compressibility_coefficient,
            "viscosity_coefficient": self.viscosity_coefficient,
            "cauchy_strain": self.strain,
            "cauchy_stress": self.stress,
            "initial_amplitude": {
                "bc_xpos": self.displacement_zero,
                "bc_yneg": self.displacement_zero,
                "bc_ypos": self.displacement_zero,
                "bc_xneg": self.pressure_zero,
            },
            "normal_dir": {
                "bc_xpos": self.orientation_dir((+1, 0)),
                "bc_yneg": self.orientation_dir((0, -1)),
                "bc_ypos": self.orientation_dir((0, +1)),
                "bc_xneg": self.orientation_dir((-1, 0)),
            },
            "tangential_dir": {
                "bc_xpos": self.orientation_dir((0, +1)),
                "bc_yneg": self.orientation_dir((+1, 0)),
                "bc_ypos": self.orientation_dir((-1, 0)),
                "bc_xneg": self.orientation_dir((0, -1)),
            },
        }

    def getField(self, name, mesh_entity, pts):
        if isinstance(self.fields[name], dict):
            field = self.fields[name][mesh_entity](pts)
        else:
            field = self.fields[name](pts)
        return field

    def displacement(self, locs):
        """Compute displacement field at locations."""
        (npts, _) = locs.shape

        p_mu = p_shear_modulus
        p_lambda = p_drained_bulk_modulus - 2.0 / 3.0 * p_shear_modulus
        p_alpha = p_biot_coefficient
        x = locs[:, 0]

        disp = numpy.zeros((1, npts, self.SPACE_DIM), dtype=numpy.float64)
        ux = (
            0.5
            / (p_lambda + 2 * p_mu)
            * (1.0 - p_alpha)
            * fx
            * ((x_max - x_min) ** 2 - (x - x_min) ** 2)
        )
        disp[0, :, 0] = ux
        disp[0, :, 1] = 0.0
        return disp

    def displacement_zero(self, locs):
        (npts, _) = locs.shape

        disp = numpy.zeros((1, npts, self.SPACE_DIM), dtype=numpy.float64)
        return disp

    def pressure(self, locs):
        """Compute pressure field at locations."""
        (npts, _) = locs.shape

        x = locs[:, 0]

        pressure = numpy.zeros((1, npts, 1), dtype=numpy.float64)
        pressure[0, :, 0] = fx * (x - x_min)
        return pressure

    def pressure_zero(self, locs):
        (npts, _) = locs.shape

        pressure = numpy.zeros((1, npts, 1), dtype=numpy.float64)
        return pressure

    def trace_strain(self, locs):
        """Compute trace_strain field at locations."""
        (npts, _) = locs.shape

        x = locs[:, 0]
        p_alpha = p_biot_coefficient

        ev = -1.0 / (p_lambda + 2.0 * p_mu) * (1.0 - p_alpha) * fx * (x - x_min)

        trace = numpy.zeros((1, npts, 1), dtype=numpy.float64)
        trace[0, :, 0] = ev
        return trace

    def solid_density(self, locs):
        """Compute solid density field at locations."""
        (npts, _) = locs.shape
        density = p_solid_density * numpy.ones((1, npts, 1), dtype=numpy.float64)
        return density

    def fluid_density(self, locs):
        """Compute fluid density field at locations."""
        (npts, _) = locs.shape
        density = p_fluid_density * numpy.ones((1, npts, 1), dtype=numpy.float64)
        return density

    def fluid_viscosity(self, locs):
        """Compute fluid viscosity field at locations."""
        (npts, _) = locs.shape
        viscosity = p_fluid_viscosity * numpy.ones((1, npts, 1), dtype=numpy.float64)
        return viscosity

    def shear_modulus(self, locs):
        """Compute shear modulus field at locations."""
        (npts, _) = locs.shape
        shear_modulus = p_shear_modulus * numpy.ones((1, npts, 1), dtype=numpy.float64)
        return shear_modulus

    def drained_bulk_modulus(self, locs):
        """Compute drained bulk modulus field at locations."""
        (npts, _) = locs.shape
        bulk_modulus = p_drained_bulk_modulus * numpy.ones(
            (1, npts, 1), dtype=numpy.float64
        )
        return bulk_modulus

    def biot_coefficient(self, locs):
        """Compute Biot coefficient field at locations."""
        (npts, _) = locs.shape
        biot_coeff = p_biot_coefficient * numpy.ones((1, npts, 1), dtype=numpy.float64)
        return biot_coeff

    def biot_modulus(self, locs):
        """Compute Biot modulus field at locations."""
        (npts, _) = locs.shape
        p_solid_bulk_modulus = (
            p_drained_bulk_modulus / (1.0 - p_biot_coefficient)
            if p_biot_coefficient < 1.0
            else 1.0e50
        )
        p_biot_modulus = p_fluid_bulk_modulus / (
            p_porosity
            + (p_biot_coefficient - p_porosity)
            * p_fluid_bulk_modulus
            / p_solid_bulk_modulus
        )
        modulus = p_biot_modulus * numpy.ones((1, npts, 1), dtype=numpy.float64)
        return modulus

    def isotropic_permeability(self, locs):
        """Compute permeability field at locations."""
        (npts, _) = locs.shape
        permeability = p_isotropic_permeability * numpy.ones(
            (1, npts, 1), dtype=numpy.float64
        )
        return permeability

    def porosity(self, locs):
        """Compute porosity field at locations."""
        (npts, _) = locs.shape
        value = p_porosity * numpy.ones((1, npts, 1), dtype=numpy.float64)
        return value

    def reference_pressure(self, locs):
        """Compute reference pressure field at locations."""
        (npts, _) = locs.shape
        value = p_reference_pressure * numpy.ones((1, npts, 1), dtype=numpy.float64)
        return value

    def fluid_compressibility(self, locs):
        """Compute reference fluid compressibility field at locations."""
        (npts, _) = locs.shape
        value = p_fluid_compressibility * numpy.ones((1, npts, 1), dtype=numpy.float64)
        return value

    def fluid_compressibility_coefficient(self, locs):
        """Compute fluid compressibility coefficient field at locations."""
        (npts, _) = locs.shape
        value = p_fluid_compressibility_coefficient * numpy.ones((1, npts, 1), dtype=numpy.float64)
        return value

    def viscosity_coefficient(self, locs):
        """Compute viscosity coefficient field at locations."""
        (npts, _) = locs.shape
        value = p_viscosity_coefficient * numpy.ones((1, npts, 1), dtype=numpy.float64)
        return value

    def strain(self, locs):
        """Compute strain field at locations."""
        (npts, _) = locs.shape

        x = locs[:, 0]
        p_alpha = p_biot_coefficient

        strain = numpy.zeros((1, npts, self.TENSOR_SIZE), dtype=numpy.float64)
        exx = -1.0 / (p_lambda + 2.0 * p_mu) * (1.0 - p_alpha) * fx * (x - x_min)
        strain[0, :, 0] = exx
        strain[0, :, 1] = 0.0
        strain[0, :, 2] = 0.0
        strain[0, :, 3] = 0.0
        return strain

    def stress(self, locs):
        """Compute stress field at locations."""
        (npts, _) = locs.shape

        x = locs[:, 0]
        p_alpha = p_biot_coefficient
        sxx = -fx * (x - x_min)
        syy = szz = -p_lambda / (p_lambda + 2 * p_mu) * fx * (x - x_min) - 2 * p_mu / (
            p_lambda + 2 * p_mu
        ) * p_alpha * fx * (x - x_min)

        stress = numpy.zeros((1, npts, self.TENSOR_SIZE), dtype=numpy.float64)
        stress[0, :, 0] = sxx
        stress[0, :, 1] = syy
        stress[0, :, 2] = szz
        stress[0, :, 3] = 0.0
        return stress

    def orientation_dir(self, vector):
        def fn_dir(locs):
            (npts, _) = locs.shape
            values = numpy.zeros((1, npts, self.SPACE_DIM), dtype=numpy.float64)
            for d in range(self.SPACE_DIM):
                values[:, :, d] = vector[d]
            return values

        return fn_dir


# End of file
