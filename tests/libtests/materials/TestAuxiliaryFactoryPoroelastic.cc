...
#include "TestAuxiliaryFactoryPoroelastic.hh" // Implementation of class methods

#include "pylith/materials/AuxiliaryFactoryPoroelastic.hh" // Test subject
...
// Setup testing data.
void
pylith::materials::TestAuxiliaryFactoryPoroelastic::setUp(void) {
    PYLITH_METHOD_BEGIN;

    pythia::journal::debug_t debug("auxiliaryfactoryporoelastic");

    _data = new TestAuxiliaryFactoryPoroelastic_Data();CPPUNIT_ASSERT(_data);
    ...
    componentNames.resize(1);
    componentNames[0] = "drained_bulk_modulus";
    info.description = pylith::topology::Field::Description(
        "drained_bulk_modulus",
        "drained_bulk_modulus",
        componentNames,
    ...
    info.index = 2;
    _data->subfields["drained_bulk_modulus"] = info;
...
void
pylith::materials::TestAuxiliaryFactoryPoroelastic::tearDown(void) {
    PYLITH_METHOD_BEGIN;
...
void
pylith::materials::TestAuxiliaryFactoryPoroelastic::testAdd(void) {
    PYLITH_METHOD_BEGIN;
...
// Test setValues().
void
pylith::materials::TestAuxiliaryFactoryPoroelastic::testSetValuesFromDB(void) {
    PYLITH_METHOD_BEGIN;
...
void
pylith::materials::TestAuxiliaryFactoryPoroelastic::_initialize(void) {
    PYLITH_METHOD_BEGIN;
    ...
    _factory = new AuxiliaryFactoryPoroelastic();
...
// ---------------------------------------------------------------------------------------------------------------------
pylith::materials::TestAuxiliaryFactoryPoroelastic_Data::TestAuxiliaryFactoryPoroelastic_Data(void) :
    meshFilename(NULL),
    cs(NULL),
