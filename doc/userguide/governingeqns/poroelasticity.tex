...
\begin{gather}
% Solution
\vec{s}^{T} = \left(\vec{u}, \quad p, \quad \epsilon_v, \quad \vec{v}, \quad P_{dot}, \quad E_{dot} \right), \\
% Elasticity
...
% Press. Accel
\dot{p} - P_{dot} = 0 \text{ in } \Omega \\
% Vol. Strain Accel
\dot{\epsilon}_{v} - E_{dot} = 0 \text{ in } \Omega \\
% Neumann traction
...
functions $\trialvec[u]$, $\trialscalar[p]$, $\trialscalar[\epsilon_{v}]$, $\trialvec[v]$, $\trialscalar[P_{dot}]$, and $\trialscalar[E_{dot}]$,
...
% Weak Pressure Acceleration
\int_{\Omega} \trialscalar[P_{dot}] \left( \dot{p} - P_{dot} \right) \, d\Omega = 0, \\
% Weak Volumetric Strain Acceleration
\int_{\Omega} \trialscalar[E_{dot}] \left( \dot{\epsilon}_{v} - E_{dot} \right) \, d\Omega = 0.
\end{gather}
%
...
\int_{\Omega} \trialscalar[P_{dot}] \left(  \dot{p} - P_{dot} \right) \, d\Omega = 0 \\
% Weak Volumetric Strain Acceleration
\int_{\Omega} \trialscalar[E_{dot}] \left( \dot{\epsilon_{v}} - E_{dot} \right) \, d\Omega = 0
\end{gather}
...
\begin{align}
  ...
  F^{P_{dot}}(t,s,\dot{s}) &= \int_{\Omega} \trialscalar[P_{dot}] \eqnannotate{\left[ \dot{p} - P_{dot} \right]}{ f^{ P_{dot} }_{0} } \, d\Omega, \\
  % Volumetric Strain Acceleration
  F^{E_{dot}}(t,s,\dot{s}) &= \int_{\Omega} \trialscalar[E_{dot}] \eqnannotate{\left[ \dot{\epsilon}_{v} - E_{dot} \right]}{f^{E_{dot}}_{0}} \, d\Omega. \\
\end{align}
...
%
% JF_uP
J_F^{u P_{dot}} &= \frac{\partial F^u}{\partial P_{dot}} + s_{tshift} \frac{\partial F^u}{\partial \dot{P}_{dot}} = 0 \\
%
% JF_uE
J_F^{u E_{dot}} &= \frac{\partial F^u}{\partial E_{dot}} + s_{tshift} \frac{\partial F^u}{\partial \dot{E}_{dot}} = 0 
%
\end{align}
...
% 
% JF_pP
J_F^{p P_{dot}} = \frac{\partial F^p}{\partial P_{dot}} + s_{tshift} \frac{\partial F^p}{\partial \dot{P}_{dot}} &= 
...
&= \int_{\Omega} \trialscalar[p] \eqnannotate{\left[ \frac{1}{M} \right]}{J_{f0}^{pP_{dot}}} \basisscalar[P_{dot}] \ d\Omega \\
%
% JF_pE
J_F^{p E_{dot}} = \frac{\partial F^p}{\partial E_{dot}} + s_{tshift} \frac{\partial F^p}{\partial \dot{E}_{dot}} &= 
...
&= \int_{\Omega} \trialscalar[p] \eqnannotate{\left[ \alpha \right]}{J_{f0}^{pE_{dot}}} \basisscalar[E_{dot}] \ d\Omega \\
%
\end{align}
...
% JF_eP
% 
J_F^{\epsilon_{v} P_{dot}} = \frac{\partial F^{\epsilon_{v}}}{\partial P_{dot}} + s_{tshift} \frac{\partial F^{\epsilon_{v}}}{\partial \dot{P}_{dot}} &= 0 \\
% JF_eE
%
J_F^{\epsilon_{v} E_{dot}} = \frac{\partial F^{\epsilon_{v}}}{\partial E_{dot}} + s_{tshift} \frac{\partial F^{\epsilon_{v}}}{\partial \dot{E}_{dot}} &= 0
%
\end{align}
...
% JF_VP
%
J_F^{v P_{dot}} = \frac{\partial F^{v}}{\partial P_{dot}} + s_{tshift} \frac{\partial F^{v}}{\partial \dot{P}_{dot}} &= 0 \\
%
% JF_VE
%
J_F^{v E_{dot}} = \frac{\partial F^{v}}{\partial E_{dot}} + s_{tshift} \frac{\partial F^{v}}{\partial \dot{E}_{dot}} &= 0
%
\end{align}
...
%
% JF_PU
J_F^{P_{dot} u} = \frac{\partial F^{P_{dot}}}{\partial u} + s_{tshift} \frac{\partial F^{P_{dot}}}{\partial \dot{u}} &= 0 \\
%
% JF_Pp
%
J_F^{P_{dot} p} = \frac{\partial F^{P_{dot}}}{\partial p} + s_{tshift} \frac{\partial F^{P_{dot}}}{\partial \dot{p}} &= 
s_{tshift} \int_{\Omega} \trialscalar[P_{dot}] \frac{\partial}{\partial \dot{p}} \left[ \dot{p} - P_{dot} \right] \ d\Omega \\
&= \int_{\Omega} \trialscalar[P_{dot}] \eqnannotate{\left[s_{tshift} \right]}{J_{f0}^{P_{dot} p}} \basisscalar[p] \ d\Omega \\
% JF_Pe
%
J_F^{P_{dot} \epsilon_{v}} = \frac{\partial F^{P_{dot}}}{\partial \epsilon_{v}} + s_{tshift} \frac{\partial F^{P_{dot}}}{\partial \dot{\epsilon}_{v}} &= 0 \\
%
% JF_PV
%
J_F^{P_{dot} v} = \frac{\partial F^{P_{dot}}}{\partial v} + s_{tshift} \frac{\partial F^{P_{dot}}}{\partial \dot{v}} &= 0 \\
%
% JF_PP
%
J_F^{P_{dot} P_{dot}} = \frac{\partial F^{P_{dot}}}{\partial P_{dot}} + s_{tshift} \frac{\partial F^{P_{dot}}}{\partial \dot{P}_{dot}} &= 
   \int_{\Omega} \trialscalar[P_{dot}] \frac{\partial}{\partial P_{dot}} \left[ \dot{p} - P_{dot} \right] \ d\Omega \\
&= \int_{\Omega} \trialscalar[P_{dot}] \eqnannotate{\left[ -1 \right]}{J_{f0}^{P_{dot} P_{dot}}} \basisscalar[p] \ d\Omega \\
%
% JF_PE
%
J_F^{P_{dot} E_{dot}} = \frac{\partial F^{P_{dot}}}{\partial E_{dot}} + s_{tshift} \frac{\partial F^{P_{dot}}}{\partial \dot{E}_{dot}} &= 0 
%
\end{align}
...
% JF_EU
% 
J_F^{E_{dot} u} = \frac{\partial F^{E_{dot}}}{\partial u} + s_{tshift} \frac{\partial F^{ E_{dot}}}{\partial \dot{u}} &= 0 \\
%
% JF_Ep
%
J_F^{E_{dot} p} = \frac{\partial F^{E_{dot}}}{\partial p} + s_{tshift} \frac{\partial F^{ E_{dot}}}{\partial \dot{p}} &= 0 \\
%
% JF_Ee
%
J_F^{E_{dot} \epsilon_{v}} = \frac{\partial F^{E_{dot}}}{\partial E_{dot}} + s_{tshift} \frac{\partial F^{ E_{dot}}}{\partial \dot{E}_{dot}} &= 
   s_{tshift} \int_{\Omega} \trialscalar[E_{dot}] \frac{\partial}{\partial \dot{\epsilon}_{v}} \left[ \dot{\epsilon}_{v} - E_{dot} \right] \ d\Omega \\
&= \int_{\Omega} \trialscalar[E_{dot}] \eqnannotate{\left[s_{tshift} \right]}{J_{f0}^{E_{dot} \epsilon_{v}}} \basisscalar[\epsilon_{v}] \ d\Omega \\
% 
% JF_EV
%
J_F^{E_{dot} v} = \frac{\partial F^{E_{dot}}}{\partial v} + s_{tshift} \frac{\partial F^{ E_{dot} }}{\partial \dot{v}} &= 0 \\
% 
% JF_EP
%
J_F^{E_{dot} P_{dot}} = \frac{\partial F^{E_{dot}}}{\partial P_{dot}} + s_{tshift} \frac{\partial F^{ E_{dot} }}{\partial \dot{P_{dot}}} &= 0 \\
% 
% JF_EE
%
J_F^{E_{dot} E_{dot}} = \frac{\partial F^{E_{dot}}}{\partial E_{dot}} + s_{tshift} \frac{\partial F^{ E_{dot} }}{\partial \dot{E}_{dot}} &= 
   \int_{\Omega} \trialscalar[E_{dot}] \frac{\partial}{\partial E_{dot}} \left[ \dot{\epsilon}_{v} - E_{dot} \right] \ d\Omega \\
&= \int_{\Omega} \trialscalar[E_{dot}] \eqnannotate{\left[ -1 \right]}{J_{f0}^{E_{dot} E_{dot}}} \basisscalar[p] \ d\Omega \\
% 
\end{align}
