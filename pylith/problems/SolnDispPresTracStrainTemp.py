...
class SolnDispPresTracStrainTemp(PetscComponent):
    """
    Container for solution subfields with displacement, pressure, trace_strain, and temperature.
    
    This is used for quasistatic thermoporoelasticity problems (coupled THM).
    """
    DOC_CONFIG = {
        "cfg": """
            [pylithapp.problem]
            solution = pylith.problems.SolnDispPresTracStrainTemp
        """
    }
    ...
    def __init__(self, name="solndispprestracstraintemp"):
        """Constructor.
        """
    ...
    def components(self):
        ...
        components() to insure order is [displacement, pressure, trace_strain, temperature].

        """
        return [self.displacement, self.pressure, self.traceStrain, self.temperature]


class Solution(SolutionBase):
    """Python solution field with displacement, pressure, trace_strain, and temperature subfields.
    """
    ...
    subfields = pythia.pyre.inventory.facilityArray(
        "subfields", family="soln_subfields", itemFactory=subfieldFactory, factory=SolnDispPresTracStrainTemp)
