...
class SolnDispPresTemp(PetscComponent):
    """
    Container for solution subfields with displacement, pressure, and temperature subfields.
    
    This is used for porothermoelasticity problems (coupled thermal-hydraulic-mechanical).
    """
    DOC_CONFIG = {
        "cfg": """
            [pylithapp.problem]
            solution = pylith.problems.SolnDispPresTemp
        """
    }
    ...
    def __init__(self, name="solndispprestep"):
        """Constructor.
        """
    ...
    def components(self):
        ...
        components() to insure order is [displacement, pressure, temperature].

        """
        return [self.displacement, self.pressure, self.temperature]


class Solution(SolutionBase):
    """Python solution field with displacement, pressure, and temperature subfields.
    """
    ...
    subfields = pythia.pyre.inventory.facilityArray(
        "subfields", family="soln_subfields", itemFactory=subfieldFactory, factory=SolnDispPresTemp)
