#!/usr/bin/env nemesis
"""Generate a tri mesh for earthquake thermoelasticity example using Gmsh.

This mesh represents a vertical cross-section through the crust with a 
strike-slip fault. The domain is designed to demonstrate:
- Thermoelastic coupling during an earthquake
- Shear heating from fault slip
- Heat conduction away from the fault
...
class App(GenerateMesh):
    ...
    def mark(self):
        ...
        vertex_groups = (
            VertexGroup(name="fault_edge", tag=21, dim=0, entities=[]),
        )
        # Get the fault tip point tag - we need to find it from the fault curve
        gmsh.model.geo.synchronize()
        # The fault tip is at the end of c_fault (point p7)
        # We'll mark it using the boundary of the fault curve
        fault_boundary = gmsh.model.getBoundary([(1, self.c_fault)], oriented=False)
        # fault_boundary contains the two endpoints of the fault line
        # We want the one at depth (not at surface)
