#!/usr/bin/env nemesis
"""Generate a tri mesh for thermoporoelasticity examples using Gmsh.

This mesh represents a vertical cross-section for coupled THM simulations.
The domain includes an optional fault for demonstrating fault-THM coupling.

Domain: 20 km wide x 10 km deep
- -10.0 km <= x <= 10.0 km
- -10.0 km <= y <= 0.0 km (y=0 is the surface)

The fault runs vertically through the center from the surface to 7.5 km depth.
...
class App(GenerateMesh):
    ...
    """
    DOMAIN_X = 20.0e+3  # 20 km wide
    DOMAIN_Y = 10.0e+3  # 10 km deep
    FAULT_DEPTH = 7.5e+3  # Fault extends to 7.5 km depth

    DX_FAULT = 250.0  # 250 m resolution near fault
    DX_BIAS = 1.12  # Geometric progression bias
    ...
    def create_geometry(self):
        ...
        p5 = gmsh.model.geo.add_point(0.0, y1, 0.0)  # Fault at bottom (mesh transition)
        p6 = gmsh.model.geo.add_point(0.0, 0.0, 0.0)  # Fault at surface
        p7 = gmsh.model.geo.add_point(0.0, -fault_depth, 0.0)  # Fault tip

        # Create curves
        # Bottom boundary (in two parts)
